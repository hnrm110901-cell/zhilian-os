...
class VoiceCommandService:
    ...
    def __init__(self, redis_client=None):
        ...
        self.intent_patterns = {
            ...
            VoiceIntent.CALL_SUPPORT: [r"(呼叫|叫|需要).*(支援|帮忙|帮助)", r"(人手|人员).*(不够|不足)", r"(忙不过来|太忙)"],
        }
            ...
            for intent, patterns in self.intent_patterns.items():
                for pattern in patterns:
                    if re.search(pattern, voice_text):
                        logger.info("intent_recognized", voice_text=voice_text, intent=intent, pattern=pattern)
                        return intent
    ...
    async def _handle_queue_status(self, store_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            count_result = await db.execute(
                ...
                .select_from(Queue)
                .where(Queue.store_id == store_id, Queue.status == QueueStatus.WAITING)
            )
            ...
            if waiting_queues == 0:
                ...
                avg_result = await db.execute(
                    select(sa_func.avg(Queue.actual_wait_time)).where(
                        Queue.store_id == store_id, Queue.actual_wait_time.isnot(None)
                    )
                )
            ...
            return {
                ...
                "data": {"waiting_count": waiting_queues, "estimated_wait_time": avg_wait_time if waiting_queues > 0 else 0},
            ...
            return {"success": False, "message": f"查询排队状态失败: {str(e)}", "voice_response": "抱歉，查询排队状态失败"}
    ...
    async def _handle_order_reminder(self, store_id: str, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            if not timeout_orders:
                ...
                # 找出最长等待时间
                max_wait = max([(datetime.utcnow() - order.created_at).total_seconds() / 60 for order in timeout_orders])
            ...
            return {"success": False, "message": f"查询超时订单失败: {str(e)}", "voice_response": "抱歉，查询超时订单失败"}

    async def _handle_inventory_query(self, store_id: str, voice_text: str, db: AsyncSession) -> Dict[str, Any]:
        """处理库存查询"""
        try:
            # 从语音文本中提取物品名称
            # 简单实现：查询所有低库存物品
            result = await db.execute(
                select(InventoryItem).where(
                    InventoryItem.store_id == store_id, InventoryItem.quantity < InventoryItem.min_quantity
                )
            )
            ...
            return {
                ...
                "data": {
                    ...
                    "low_stock_items": [
                        {"item_id": item.id, "name": item.name, "quantity": item.quantity, "min_quantity": item.min_quantity}
            ...
            return {"success": False, "message": f"查询库存失败: {str(e)}", "voice_response": "抱歉，查询库存失败"}
        ...
        try:
            ...
            today_rev = await db.execute(
                select(func.sum(Order.total_amount)).where(
                    Order.store_id == store_id, Order.created_at >= today_start, Order.status.in_(["completed", "paid"])
                )
            )
            ...
            return {"success": False, "message": f"查询今日营收失败: {str(e)}", "voice_response": "抱歉，查询今日营收失败"}

    async def _handle_call_support(self, store_id: str, user_id: str, db: AsyncSession) -> Dict[str, Any]:
        """处理呼叫支援"""
        try:
            # 获取门店信息
            ...
            logger.info("support_request_sent", store_id=store_id, user_id=user_id)
            ...
            return {"success": False, "message": f"发送支援请求失败: {str(e)}", "voice_response": "抱歉，发送支援请求失败"}

    async def broadcast_meituan_queue_update(
        self, store_id: str, queue_count: int, estimated_wait_time: int
    ) -> Dict[str, Any]:
        """
        广播美团排队更新（每5分钟自动播报）
        ...
        try:
            ...
            logger.info(
                "meituan_queue_broadcast", store_id=store_id, queue_count=queue_count, estimated_wait_time=estimated_wait_time
            )

            return {
                "success": True,
                "type": "broadcast",
                "data": {"queue_count": queue_count, "estimated_wait_time": estimated_wait_time},
                "voice_response": voice_response,
            }

        except Exception as e:
            logger.error("broadcast_meituan_queue_failed", error=str(e))
            return {"success": False, "message": f"广播美团排队失败: {str(e)}"}

    async def alert_timeout_order(self, store_id: str, table_number: str, wait_time: int) -> Dict[str, Any]:
        """
        超时订单告警（自动播报）
        ...
        try:
            ...
            logger.warning("timeout_order_alert", store_id=store_id, table_number=table_number, wait_time=wait_time)

            return {
                "success": True,
                "type": "alert",
                "data": {"table_number": table_number, "wait_time": wait_time},
                "voice_response": voice_response,
            }

        except Exception as e:
            logger.error("alert_timeout_order_failed", error=str(e))
            return {"success": False, "message": f"发送超时告警失败: {str(e)}"}
