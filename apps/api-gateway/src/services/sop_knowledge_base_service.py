...
class SOPKnowledgeBaseService:
    ...
    def _initialize_knowledge_base(self):
        ...
        self._add_sop(
            ...
            """,
                ...
                best_practices=["保持微笑和耐心", "不要争辩或推卸责任", "快速响应，不超过2分钟", "给予实质性补偿"],
                common_mistakes=["推卸责任给后厨", "质疑顾客的口味", "拖延处理时间", "补偿不够诚意"],
        ...
        self._add_sop(
            ...
            """,
                ...
                best_practices=["提前15分钟预判高峰", "保持冷静，有序指挥", "优先保证食品安全", "事后给员工加餐或奖励"],
                common_mistakes=["慌乱无序", "忽视食品安全", "对顾客态度不佳", "事后不复盘"],
        ...
        self._add_sop(
            ...
            """,
                ...
                best_practices=["使用检查表，逐项打勾", "拍照留存证据", "发现问题立即整改", "定期培训员工"],
                common_mistakes=["走过场，不认真检查", "发现问题不上报", "检查记录造假", "忽视小问题"],
        ...
        self._add_sop(
            ...
            """,
                ...
                best_practices=["建立维护台账", "使用专业工具", "定期更换易损件", "培训专人负责"],
                common_mistakes=["清洗不彻底", "忘记断电", "使用腐蚀性清洗剂", "安装不到位"],
    ...
    async def query_best_practice(self, query: str, context: QueryContext) -> List[SOPRecommendation]:
        """
        查询最佳实践
    ...
    def _search_by_keywords(self, query: str) -> List[SOPDocument]:
        ...
        # 返回匹配的SOP
        return [self.sop_database[sop_id] for sop_id in matched_sop_ids]

    def _calculate_relevance(self, sop: SOPDocument, query: str, context: QueryContext) -> float:
        """计算相关性分数"""
        score = 0.0
        ...
        # 使用次数加权
        score += min(
            sop.usage_count / float(os.getenv("SOP_SCORE_USAGE_DIVISOR", "1000")),
            float(os.getenv("SOP_SCORE_USAGE_MAX", "0.2")),
        )

        return min(score, 1.0)

    def _calculate_confidence(self, sop: SOPDocument, context: QueryContext) -> float:
        """计算置信度"""
    ...
    async def add_custom_sop(self, store_id: str, sop: SOPDocument) -> Dict[str, Any]:
        """
        添加自定义SOP
        ...
        return {"sop_id": sop.sop_id, "status": "success", "message": "SOP已添加到知识库"}

    async def rate_sop(self, sop_id: str, rating: float, feedback: Optional[str] = None) -> Dict[str, Any]:
        """
        评价SOP
        ...
        if not sop:
            return {"status": "error", "message": "SOP不存在"}

        # 更新评分（加权平均）
        total_ratings = sop.usage_count
        new_rating = (sop.rating * total_ratings + rating) / (total_ratings + 1)
        ...
        return {"sop_id": sop_id, "new_rating": new_rating, "total_ratings": sop.usage_count, "status": "success"}
    ...
    def _get_top_rated(self, limit: int) -> List[Dict]:
        """获取评分最高的SOP"""
        sorted_sops = sorted(self.sop_database.values(), key=lambda x: x.rating, reverse=True)
        return [{"sop_id": sop.sop_id, "title": sop.title, "rating": sop.rating} for sop in sorted_sops[:limit]]

    def _get_most_used(self, limit: int) -> List[Dict]:
        """获取使用最多的SOP"""
        sorted_sops = sorted(self.sop_database.values(), key=lambda x: x.usage_count, reverse=True)
        return [{"sop_id": sop.sop_id, "title": sop.title, "usage_count": sop.usage_count} for sop in sorted_sops[:limit]]
