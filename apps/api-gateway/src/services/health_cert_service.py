...
class HealthCertService:
    ...
    async def update_certificate(
        ...
        result = await session.execute(
            select(HealthCertificate).where(HealthCertificate.id == uuid.UUID(cert_id))
        )
        ...
        updatable_fields = [
            "employee_name", "certificate_number", "issue_date", "expiry_date",
            "issuing_authority", "certificate_image_url", "status",
            "physical_exam_date", "physical_exam_result", "notes", "store_id",
        ]
    ...
    async def list_certificates(
        ...
        count_query = select(func.count(HealthCertificate.id)).where(
        ...
        if employee_name:
            ...
            count_query = count_query.where(
                HealthCertificate.employee_name.contains(employee_name)
            )
    ...
    async def check_expiring(
        session: AsyncSession, brand_id: str, days_ahead: int = 30
    ) -> List[Dict[str, Any]]:
        """查询即将到期的健康证（N天内到期）"""
        from src.models.health_certificate import HealthCertificate
        ...
        result = await session.execute(
            select(HealthCertificate).where(
                HealthCertificate.brand_id == brand_id,
    ...
    async def auto_update_status(session: AsyncSession, brand_id: str) -> Dict[str, Any]:
        ...
        res1 = await session.execute(
            update(HealthCertificate).where(
                HealthCertificate.brand_id == brand_id,
