...
async def compute_tax_calculation(
    ...
    if not force:
        ...
        """), {"sid": store_id, "period": period, "today": today})).scalar() or 0
        if existing > 0:
            rows = (await db.execute(text("""
                SELECT * FROM tax_calculations
                WHERE store_id = :sid AND period = :period AND calc_date = :today
                ORDER BY tax_type
            """), {"sid": store_id, "period": period, "today": today})).fetchall()
    ...
    declared_row = (await db.execute(text("""
    ...
    """), {"sid": store_id, "today": today})).fetchall()
    ...
    for tax_type in tax_types_to_calc:
        ...
        """), {"sid": store_id, "period": period, "tt": tax_type})).fetchone()
        ...
        if existing_row:
            await db.execute(text("""
                ...
                WHERE store_id = :sid AND period = :period AND tax_type = :tt
            """), {
                "today": today, "tn": calc["tax_name"], "rate": calc["tax_rate"],
                "base": calc["taxable_base"], "amt": calc["tax_amount"],
                "decl": declared, "dev": round(deviation, 2),
                "devpct": round(deviation_pct, 2), "risk": risk_level,
                "detail": detail_json, "sid": store_id, "period": period, "tt": tax_type,
            })
        else:
            tid = str(uuid.uuid4())
            await db.execute(text("""
            ...
            """), {
                "id": tid, "sid": store_id, "period": period, "today": today,
                "tt": tax_type, "tn": calc["tax_name"], "rate": calc["tax_rate"],
                "base": calc["taxable_base"], "amt": calc["tax_amount"],
                "decl": declared, "dev": round(deviation, 2),
                "devpct": round(deviation_pct, 2), "risk": risk_level,
                "detail": detail_json,
    ...
    logger.info("tax_calculated", store_id=store_id, period=period,
                types=len(results))
    return results
...
async def _log_agent_action(
    ...
    """), {
        "id": aid, "sid": store_id, "level": level, "agent": agent_name,
        "trigger": trigger, "title": title, "desc": description,
        "impact": round(expected_impact, 2), "period": period,
    })
