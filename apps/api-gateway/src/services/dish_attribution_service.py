...
def classify_driver(price_effect: float, volume_effect: float, interaction: float, revenue_delta: float) -> str:
    """
    识别营收变化的主要驱动因子。
...
async def _fetch_period_data(db: AsyncSession, store_id: str, period: str) -> list:
    """拉取指定期的所有菜品盈利数据。"""
    sql = text("""
...
async def compute_revenue_attribution(db: AsyncSession, store_id: str, period: str, prev_period: Optional[str] = None) -> dict:
    """
    ...
    for dish_id in sorted(both):
        ...
        driver_counts[rec["primary_driver"]] = driver_counts.get(rec["primary_driver"], 0) + 1
...
async def get_revenue_attribution(
    db: AsyncSession, store_id: str, period: str, driver: Optional[str] = None, limit: int = 100
) -> list[dict]:
    """查询归因明细列表。L011 两路分支。"""
    _cols = """
    ...
    if driver:
        sql = text(f"""
            ...
            LIMIT :limit
        """)
        params = {"store_id": store_id, "period": period, "driver": driver, "limit": limit}
    else:
        sql = text(f"""
            SELECT {_cols}
            FROM dish_revenue_attribution
...
async def get_attribution_summary(db: AsyncSession, store_id: str, period: str) -> dict:
    """按主要驱动因子聚合统计。"""
    sql = text("""
        SELECT
            primary_driver,
...
async def get_top_movers(db: AsyncSession, store_id: str, period: str, direction: str = "gain", limit: int = 10) -> list[dict]:
    """
...
async def get_dish_attribution_history(db: AsyncSession, store_id: str, dish_id: str, periods: int = 6) -> list[dict]:
    """某道菜近 N 期的 PVM 归因历史。"""
    sql = text("""
        SELECT
            period, prev_period,
