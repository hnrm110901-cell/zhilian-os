...
class CrossStoreInsightsService:
    ...
    async def generate_insight_summary(
        ...
        if anomalies:
            lines = [
                f"- {a['store_name']}：{a['metric']} {a['direction']} 均值 {abs(a['z_score'])} 个标准差（值={a['value']}，均值={a['mean']}）"
        ...
        if anomalies:
            parts.append(
                f"发现 {len(anomalies)} 家门店 {metric} 异常，其中 {anomalies[0].get('store_name', '')} 偏差最大（z={anomalies[0]['z_score']}）。"
            )
        if best_practices.get("top_stores"):
            top = best_practices["top_stores"][0]
            parts.append(
                f"{metric} 最佳门店为 {top.get('store_name', top['store_id'])}（{top.get('value', 0)}），可作为标杆参考。"
            )
        if period_comparison:
