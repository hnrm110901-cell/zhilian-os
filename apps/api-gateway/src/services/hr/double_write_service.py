...
class DoubleWriteService:
    ...
    async def _do_create(self, emp) -> bool:
        ...
        async with self._session_factory() as session:
            # Check idempotency
            existing = await self._lookup_id_map(session, emp.id)
            ...
            org_node_id = await self._get_org_node_for_employee(session, emp)
            ...
            for skill_str in all_skill_strings:
                ...
                safe_str = skill_str.replace("\\", "\\\\").replace("%", "\\%").replace("_", "\\_")
                pattern = f"%{safe_str}%"
                result = await session.execute(
                    sa.text(
                        "SELECT id FROM skill_nodes "
                        "WHERE skill_name ILIKE :pattern ESCAPE '\\'"
                    ),
                    {"pattern": pattern},
                )
    ...
    async def _do_update(self, emp) -> bool:
        ...
        async with self._session_factory() as session:
            id_map = await self._lookup_id_map_full(session, emp.id)
            if id_map is None:
                logger.info(
    ...
    async def _lookup_id_map(self, session: AsyncSession, employee_id: str):
    ...
    async def _lookup_id_map_full(self, session: AsyncSession, employee_id: str):
    ...
    async def _get_org_node_for_employee(self, session: AsyncSession, emp) -> Optional[str]:
