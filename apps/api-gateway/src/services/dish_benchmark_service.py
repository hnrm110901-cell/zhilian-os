...
def compute_cross_store_rank(value: float, all_values: list[float],
...
def compute_cross_store_percentile(value: float, all_values: list[float],
...
def find_best_store(store_values: list[tuple[str, float]],
                    higher_is_better: bool = True) -> tuple[Optional[str], float]:
...
def compute_gap_pp(store_value: float, best_value: float,
                   higher_is_better: bool = True) -> float:
...
def build_dish_benchmark_records(period: str, dish_name: str,
                                  store_data: list[dict]) -> list[dict]:
    """
    ...
    best_fcr_store, best_fcr_val = find_best_store(
        [(d['store_id'], d['food_cost_rate']) for d in store_data],
        higher_is_better=False
    )
    best_gpm_store, best_gpm_val = find_best_store(
        [(d['store_id'], d['gross_profit_margin']) for d in store_data],
...
async def get_dish_benchmark(db: AsyncSession, store_id: str, period: str,
                              fcr_tier: Optional[str] = None,
...
async def get_store_benchmark_summary(db: AsyncSession, store_id: str,
                                       period: str) -> dict:
...
async def get_laggard_dishes(db: AsyncSession, store_id: str, period: str,
    ...
    sql = text("""
        ...
        LIMIT :limit
    """)
    rows = (await db.execute(sql, {'store_id': store_id, 'period': period,
                                    'limit': limit})).fetchall()
    cols = [
        'dish_name', 'food_cost_rate', 'gross_profit_margin', 'order_count',
        'revenue_yuan', 'fcr_rank', 'store_count',
        'best_fcr_value', 'best_fcr_store_id',
        'fcr_gap_pp', 'fcr_gap_yuan_impact',
        'best_gpm_value', 'best_gpm_store_id',
...
async def get_benchmark_trend(db: AsyncSession, store_id: str, period: str,
    ...
    rows = (await db.execute(sql, {'store_id': store_id,
                                    'start': start, 'period': period})).fetchall()
    return [
        {
            'period':              r[0],
            'dish_count':          int(r[1]),
...
async def get_dish_cross_store_detail(db: AsyncSession, dish_name: str,
