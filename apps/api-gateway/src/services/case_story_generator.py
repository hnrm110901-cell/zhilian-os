...
async def _query_revenue(db: AsyncSession, store_id: str, start: date, end: date) -> int:
    """返回区间内的总营业额（分）"""
...
async def _query_actual_cost(db: AsyncSession, store_id: str, start: date, end: date) -> int:
...
async def _query_waste_cost(db: AsyncSession, store_id: str, start: date, end: date) -> int:
...
async def _query_decisions(db: AsyncSession, store_id: str, start: date, end: date) -> List[Dict[str, Any]]:
...
def _compute_cost_metrics(
    ...
    actual_cost_pct = round(actual_cost_fen / revenue_fen * 100, 2) if revenue_fen > 0 else 0.0
    waste_pct = round(waste_cost_fen / revenue_fen * 100, 2) if revenue_fen > 0 else 0.0
...
def _summarize_decisions(decisions: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    total_saving_yuan = sum(d["expected_saving_yuan"] for d in decisions if d["status"] == "APPROVED")
...
def _narrative_sentence(
    ...
    if cost_metrics["waste_cost_yuan"] > 0:
        parts.append(f"损耗成本 ¥{cost_metrics['waste_cost_yuan']}，占营收 {cost_metrics['waste_pct']}%。")
...
class CaseStoryGenerator:
    ...
    async def generate_daily_story(
        ...
        return {
            ...
            "decisions": decisions,
            "narrative": _narrative_sentence(store_id, period_label, cost_metrics, decision_summary),
    ...
    async def generate_weekly_story(
        ...
        # 环比计算（避免除零）
        revenue_wow = round((revenue_fen - prev_revenue_fen) / prev_revenue_fen * 100, 1) if prev_revenue_fen > 0 else 0.0
        prev_cost_pct = round(prev_actual_cost_fen / prev_revenue_fen * 100, 2) if prev_revenue_fen > 0 else 0.0
