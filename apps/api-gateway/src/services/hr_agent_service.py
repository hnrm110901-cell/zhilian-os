...
class HRAgentService:
    ...
    async def analyze_payroll_anomalies(
        ...
        for emp_id, cur in current.items():
            ...
            if emp_id in prev_data and prev_data[emp_id] > 0:
                ...
                if abs(change_pct) > 20:
                    reasons.append(
                        f"环比{'增长' if change_pct > 0 else '下降'}{abs(change_pct):.0f}%"
                    )
    ...
    def _payroll_suggestion(
        self, anomalies: list, overtime_yuan: float, total_yuan: float
    ) -> str:
        """生成薪资优化建议"""
        suggestions = []
        ...
        if ot_ratio > 15:
            ...
            suggestions.append(
                f"加班费占比{ot_ratio:.0f}%偏高，建议优化排班。"
                f"若降至10%可节省约¥{savings:,.0f}/月"
            )
        if len(anomalies) > 3:
            suggestions.append(
                f"有{len(anomalies)}名员工薪资异常波动，建议HR核查"
            )
        return " | ".join(suggestions) if suggestions else "本月薪资结构正常，未发现明显异常"

    # ── 2. 离职预警 ────────────────────────────────────────

    async def predict_resignation_risk(
        ...
        # 信号2: 合同即将到期
        contract_result = await db.execute(text("""
        ...
        # 信号3: 近期请假频繁
        leave_result = await db.execute(text("""
        ...
        for emp_id, data in sorted(risk_scores.items(), key=lambda x: -x[1]["score"]):
            ...
            if data["score"] >= 50:
                try:
                    ai_result = await _ai_decision_service.predict_turnover_risk(
                        db, emp_id, self.store_id
                    )
                except Exception as e:
                    logger.warning(
                        "hr_agent_ai_prediction_failed",
                        employee_id=emp_id, error=str(e),
                    )
    ...
    async def recommend_recruitment(
        self, db: AsyncSession
    ) -> Dict[str, Any]:
        """
        招聘策略推荐：
    ...
    def _recruitment_suggestion(self, recommendations: list, channels: list) -> str:
        ...
        if urgent:
            return (
                f"有{len(urgent)}个岗位招聘紧急，"
                f"建议加大{', '.join(urgent[0]['suggested_channels'][:2])}渠道投入"
            )
    ...
    async def analyze_growth_insights(
        self, db: AsyncSession
    ) -> Dict[str, Any]:
        """
        成长洞察分析：
        ...
        promotion_result = await db.execute(text("""
        ...
        wellbeing_result = await db.execute(text("""
        ...
        # 5. 里程碑统计（近30天）
        milestone_result = await db.execute(text("""
        ...
        return {
            ...
            "ai_suggestion": self._growth_suggestion(
                skill_bottlenecks, promotion_ready, wellbeing_summary
            ),
        }
    ...
    def _growth_suggestion(
        ...
        if bottlenecks:
            top = bottlenecks[0]
            parts.append(
                f"「{top['skill_name']}」是全店最薄弱技能（均分{top['avg_score']}），"
                f"建议优先安排培训"
            )
        if promotion_ready:
            parts.append(
                f"有{len(promotion_ready)}名员工已满足晋升条件，"
                f"及时晋升可降低离职风险"
            )
        avg_wb = wellbeing.get("avg_score", 0)
        if avg_wb > 0 and avg_wb < 6:
            parts.append(
                f"本月幸福指数均分{avg_wb}偏低，建议关注员工关怀"
            )
    ...
    async def analyze_culture_health(
        self, db: AsyncSession
    ) -> Dict[str, Any]:
        """
        文化健康度分析：
        ...
        today = date.today()
        current_period = f"{today.year}-{today.month:02d}"
        dim_result = await db.execute(text("""
        ...
        # 导师覆盖率
        mentor_result = await db.execute(text("""
            SELECT COUNT(*) AS total,
        ...
        mentor_coverage = round(
            (mr.get("with_mentor", 0) or 0) / max(mentor_total, 1) * 100, 1
        )
        ...
        return {
            ...
            "milestone_coverage_pct": round(
                (ms.get("unique_employees", 0) or 0) / total_employees * 100, 1
            ),
            "wellbeing_dimensions": dimensions,
            "weakest_dimension": weakest[0] if weakest else None,
            "mentor_coverage_pct": mentor_coverage,
            "ai_suggestion": self._culture_suggestion(
                milestone_density, dimensions, mentor_coverage, weakest
            ),
        }
    ...
    def _culture_suggestion(
        ...
        dim_names = {
            "achievement": "工作成就感", "belonging": "团队归属感",
            "growth": "成长获得感", "balance": "生活平衡感",
            "culture": "文化认同感",
        }
    ...
    async def predict_resignation_risk_enhanced(
        self, db: AsyncSession
    ) -> Dict[str, Any]:
        """
        ...
        base_risk["total_replacement_cost_yuan"] = sum(
            e["replacement_cost_yuan"] for e in base_risk["at_risk_employees"]
        )
    ...
    async def recommend_headcount_optimization(
        self, db: AsyncSession
    ) -> Dict[str, Any]:
        ...
        payroll_result = await db.execute(text("""
    ...
    async def analyze_training_roi(
        self, db: AsyncSession
    ) -> Dict[str, Any]:
        """培训投入 vs 技能提升 vs 绩效变化 → ¥影响量化"""
        # 培训完成数据
        training_result = await db.execute(text("""
        ...
        # 师徒制完成数据
        mentorship_result = await db.execute(text("""
