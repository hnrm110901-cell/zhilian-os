...
def compute_profitability_score(gpm: float, avg_gpm: float, food_cost_rate: float, avg_fcr: float) -> float:
    """
    盈利能力评分 (0–25)。
...
def compute_growth_score(revenue_trend_pct: float, order_trend_pct: float, lifecycle_phase: str) -> float:
    """
    成长性评分 (0–25)。
    生命周期阶段基础分 + 趋势均值修正 ±3。
    """
...
def find_top_components(p: float, g: float, b: float, f: float) -> tuple[str, str]:
...
def build_health_score_record(
    ...
    if priority == "promote":
        ...
        action_label, action_desc = WEAKNESS_ACTIONS.get(weakness, ("持续优化", "保持现有运营策略"))
...
async def _fetch_lifecycle(db: AsyncSession, store_id: str, period: str) -> dict[str, dict]:
    sql = text("""
...
async def _fetch_benchmarks(db: AsyncSession, store_id: str, period: str) -> dict[str, str]:
...
async def _fetch_forecast_periods(db: AsyncSession, store_id: str, period: str) -> dict[str, int]:
...
async def compute_health_scores(db: AsyncSession, store_id: str, period: str) -> dict:
    """
...
async def get_health_scores(
    db: AsyncSession, store_id: str, period: str, health_tier: Optional[str] = None, limit: int = 100
) -> list[dict]:
    """查询健康评分列表。L011 两路分支。"""
    _cols = """
    ...
    if health_tier:
        sql = text(f"""
            ...
            LIMIT :limit
        """)
        params = {"store_id": store_id, "period": period, "health_tier": health_tier, "limit": limit}
    else:
        sql = text(f"""
            SELECT {_cols}
            FROM dish_health_scores
...
async def get_health_summary(db: AsyncSession, store_id: str, period: str) -> dict:
    """按 health_tier 聚合统计。"""
    sql = text("""
        SELECT
            health_tier,
...
async def get_action_priorities(
    db: AsyncSession, store_id: str, period: str, priority: str = "immediate", limit: int = 20
) -> list[dict]:
...
async def get_dish_health_history(db: AsyncSession, store_id: str, dish_id: str, periods: int = 6) -> list[dict]:
