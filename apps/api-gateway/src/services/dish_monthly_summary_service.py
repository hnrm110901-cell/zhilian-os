...
def generate_insight_text(
    ...
    # PVM 主要驱动
    _driver_labels = {
        'price': '价格变动', 'volume': '销量变动',
        'interaction': '价量联动', 'mixed': '多重因素',
    }
...
async def _fetch_profitability_summary(db: AsyncSession, store_id: str,
                                        period: str, prev_period: str) -> Optional[dict]:
    """营收基线：当期总营收 + 上期总营收。"""
    sql = text("""
        SELECT
    ...
    """)
    row = (await db.execute(sql, {'store_id': store_id,
    ...
    sql_prev = text("""
        ...
        WHERE store_id = :store_id AND period = :period
    """)
    prev_row = (await db.execute(sql_prev, {'store_id': store_id,
                                             'period': prev_period})).fetchone()
...
async def _fetch_health_summary(db: AsyncSession, store_id: str,
...
async def _fetch_matrix_summary(db: AsyncSession, store_id: str,
                                  period: str) -> Optional[dict]:
    sql = text("""
        SELECT
...
async def _fetch_attribution_summary(db: AsyncSession, store_id: str,
                                      period: str) -> Optional[dict]:
    sql = text("""
        SELECT
            COUNT(*)                   AS dish_count,
    ...
    sql_driver = text("""
        ...
        LIMIT 1
    """)
    dr = (await db.execute(sql_driver, {'store_id': store_id,
                                         'period': period})).fetchone()
    return {
...
async def _fetch_compression_summary(db: AsyncSession, store_id: str,
                                      period: str) -> Optional[dict]:
    sql = text("""
        SELECT
            COUNT(*)                                              AS dish_count,
...
async def build_dish_monthly_summary(db: AsyncSession, store_id: str,
                                      period: str) -> dict:
    """
    ...
    rev_delta_pct = compute_revenue_delta_pct(total_revenue, prev_revenue or 0) \
...
async def get_dish_monthly_summary(db: AsyncSession, store_id: str,
                                    period: str) -> Optional[dict]:
    """查询单期月度汇总。"""
    sql = text("""
        SELECT
...
async def get_summary_history(db: AsyncSession, store_id: str,
