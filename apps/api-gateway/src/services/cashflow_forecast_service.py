...
async def compute_cashflow_forecast(
    ...
    rows = (await db.execute(text("""
        ...
        GROUP BY event_type
    """), {"sid": store_id, "start": lookback_start, "today": today_str})).fetchall()
    ...
    for fc in forecasts:
        ...
        inflow_detail = json.dumps({
            "sale_est":       round(event_totals.get("sale", 0.0) / LOOKBACK_DAYS * _weekday_factor(date.fromisoformat(fc["forecast_date"])), 2),
            "collection_est": round(event_totals.get("collection", 0.0) / LOOKBACK_DAYS * _weekday_factor(date.fromisoformat(fc["forecast_date"])), 2),
        })
        outflow_detail = json.dumps({
            "purchase_est":   round(event_totals.get("purchase", 0.0) / LOOKBACK_DAYS * _weekday_factor(date.fromisoformat(fc["forecast_date"])), 2),
            "expense_est":    round(event_totals.get("expense", 0.0) / LOOKBACK_DAYS * _weekday_factor(date.fromisoformat(fc["forecast_date"])), 2),
            "payment_est":    round(event_totals.get("payment", 0.0) / LOOKBACK_DAYS * _weekday_factor(date.fromisoformat(fc["forecast_date"])), 2),
        })
        fid = str(uuid.uuid4())
        await db.execute(text("""
    ...
    if gap_days:
        ...
        await _log_cash_agent_action(
            ...
            description=f"预测最低余额 ¥{min_balance:.2f}，涉及 {len(gap_days)} 天，"
                        f"建议提前安排融资或调整付款节奏",
            expected_impact=min_balance,
        )
        await db.commit()

    logger.info("cashflow_forecasted", store_id=store_id,
                days=len(forecasts), gap_days=len(gap_days))
...
async def _load_forecast(
    ...
    """), {"sid": store_id, "gen": generated_on})).fetchall()
...
async def _log_cash_agent_action(
    ...
    """), {
        "id": aid, "sid": store_id, "level": level,
