...
class CancellationAnalyzer:
    ...
    async def analyze_cancellations(
        ...
        category_labels = {
            "price": "价格偏高", "competitor": "选择竞对", "schedule": "档期冲突",
            "quality": "服务/质量", "personal": "个人原因", "location": "位置不便",
            "capacity": "场地不合适", "other": "其他原因",
        }
    ...
    async def get_daily_summary(
        ...
        return await self.analyze_cancellations(
            session, store_id, start_date=today, end_date=today,
        )
    ...
    async def get_weekly_trend(
        ...
        analysis = await self.analyze_cancellations(
            session, store_id, start_date=week_ago, end_date=today,
        )
        ...
        analysis["daily_trend"] = [
            {"date": d, "count": c} for d, c in daily_counts.items()
        ]
        return analysis

    def _generate_insights(
        self, categories: List[Dict], total: int, total_value: int,
    ) -> List[str]:
        """生成分析洞察"""
        insights = []
        if total == 0:
        ...
        if categories:
            top = categories[0]
            insights.append(
                f"输单Top1原因：{top['label']}（{top['percentage']}%，{top['count']}笔）"
            )
    ...
    def _generate_suggestions(
        self, categories: List[Dict], competitors: List[Dict],
    ) -> List[Dict[str, str]]:
        """生成改进建议"""
        suggestions = []
