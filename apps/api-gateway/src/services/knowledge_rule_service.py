...
from src.models.knowledge_rule import IndustryBenchmark, KnowledgeRule, RuleCategory, RuleExecution, RuleStatus, RuleType
...
def _build_cross_store_rules() -> List[Dict]:
    ...
    rules = [
        ...
        (
            ...
            {
                "root_cause": "relative_underperformance",
                "confidence": 0.72,
        ...
        (
            "CROSS-002",
            "门店损耗率连续7天高于同组 p90 → 严重落后，升级为 P1 事件",
            {
                "metric": "waste_rate",
                "operator": ">",
            ...
            {
                "root_cause": "chronic_underperformance",
                "confidence": 0.85,
                "action": "启动区域总监下店辅导流程",
                "alert_level": "P1",
            },
        ...
        (
            "CROSS-003",
            "门店损耗率首次低于同组 p25 → 卓越表现，提名最佳实践",
            {"metric": "waste_rate", "operator": "<", "threshold_ref": "peer.p25", "first_time": True, "min_peer_count": 3},
            {"root_cause": "excellence_achievement", "confidence": 0.80, "action": "提名为同伴组标杆门店，整理SOP上传知识库"},
            C,
            0.80,
        ),
        (
            "CROSS-004",
            "门店损耗率下降速度低于同组中位数下降速度 → 改善迟缓",
            {
                "metric": "waste_rate_improvement_rate",
                "operator": "<",
                "threshold_ref": "peer.p50_improvement_rate",
                "window_days": 30,
            },
            {"root_cause": "slow_improvement", "confidence": 0.65, "action": "对比同组改善最快门店的行动方案"},
            C,
            0.65,
        ),
        (
            "CROSS-005",
            "海鲜品类损耗率高于同城市同类型门店均值 30% → 区域供应链异常",
            {
                "metric": "waste_rate",
                "category": "seafood",
                "operator": ">",
            ...
            {
                "root_cause": "regional_supply_chain_issue",
                "confidence": 0.76,
                "action": "联系区域采购部门排查同城配送冷链质量",
            },
            C,
            0.76,
        ),
        (
            ...
            {"root_cause": "yield_rate_gap", "confidence": 0.70, "action": "参照同组 p25 门店的切割标准重新培训"},
            C,
            0.70,
        ),
        (
            "CROSS-007",
            "全品类损耗率同时高于同组 p75 → 系统性管理问题",
            {"metric": "overall_waste_rate", "operator": ">", "threshold_ref": "peer.p75", "category_count_min": 3},
            {
                "root_cause": "systematic_management_failure",
                "confidence": 0.88,
                "action": "启动门店全面管理诊断（人员/流程/设备）",
            },
            C,
            0.88,
        ),
        (
            "CROSS-008",
            "门店损耗率在同组内排名末位超 30 天 → 触发人员调研",
            {"metric": "waste_rate_rank_in_peer", "operator": "==", "value": "last", "window_days": 30},
            {"root_cause": "persistent_bottom_performer", "confidence": 0.82, "action": "启动 360° 门店诊断，含人员访谈"},
            C,
            0.82,
        ),
        (
            "CROSS-009",
            "新开业门店 6 个月内损耗率仍高于同组 p50 → 标准化导入失败",
            {"metric": "waste_rate", "operator": ">", "threshold_ref": "peer.p50", "store_age_months_max": 6},
            {
                "root_cause": "onboarding_standardization_failure",
                "confidence": 0.74,
                "action": "安排标准化导入复盘，输出缺口清单",
            },
            C,
            0.74,
        ),
        (
            "CROSS-010",
            "节假日损耗率高于同组 p75 但平日正常 → 节假日备料预测偏差",
            {
                "metric": "waste_rate",
                "operator": ">",
                "threshold_ref": "peer.p75",
            ...
            {"root_cause": "holiday_forecast_error", "confidence": 0.68, "action": "校正节假日订货模型，参考同组历史倍增系数"},
            C,
            0.68,
        ),
        ...
        (
            "CROSS-011",
            "同菜品跨店食材用量差异 > 15% → 配方标准化漂移",
            {"metric": "bom_ingredient_variance_pct", "operator": ">", "threshold": 0.15, "min_store_count": 3},
            {
                "root_cause": "recipe_standardization_drift",
                "confidence": 0.80,
                "action": "发布配方统一令，要求各店在 7 天内对齐 BOM 主版本",
            },
            C,
            0.80,
        ),
        (
            "CROSS-012",
            "同菜品有效 BOM 版本超过 3 个 → 版本碎片化",
            {"metric": "active_bom_version_count", "operator": ">", "threshold": 3},
            {"root_cause": "bom_version_fragmentation", "confidence": 0.75, "action": "由总部厨研中心审核，指定唯一标准版本"},
            C,
            0.75,
        ),
        (
            "CROSS-013",
            "关键食材用量差异 > 10% → 核心风味一致性风险",
            {
                "metric": "key_ingredient_variance_pct",
                "operator": ">",
                "threshold": 0.10,
                "is_key_ingredient": True,
                "min_store_count": 2,
            },
            {"root_cause": "core_flavor_inconsistency", "confidence": 0.87, "action": "关键食材用量锁定，修改为强制执行字段"},
            C,
            0.87,
        ),
        (
            "CROSS-014",
            "出成率（yield_rate）在同组内差异 > 8% → 厨师技能差异显著",
            {"metric": "yield_rate_variance_pct", "operator": ">", "threshold": 0.08, "min_store_count": 3},
            {"root_cause": "chef_skill_gap", "confidence": 0.72, "action": "组织跨店厨师互相观摩，视频录制标准操作"},
            C,
            0.72,
        ),
        (
            "CROSS-015",
            "新 BOM 版本上线 30 天内，50% 门店仍未更新 → 推送阻力",
            {"metric": "bom_adoption_rate_30d", "operator": "<", "threshold": 0.50},
            {"root_cause": "bom_rollout_resistance", "confidence": 0.70, "action": "调查未更新门店原因，总部运营介入支持"},
            C,
            0.70,
        ),
        (
            "CROSS-016",
            "可选食材实际使用率差异跨店 > 40% → 区域口味偏好分化",
            {
                "metric": "optional_ingredient_usage_variance",
                "operator": ">",
                "threshold": 0.40,
                "is_optional": True,
                "min_store_count": 3,
            ...
            {
                "root_cause": "regional_taste_preference_divergence",
                "confidence": 0.65,
                "action": "允许区域化 BOM 配置，建立可选区域版本",
            },
            C,
            0.65,
        ),
        (
            "CROSS-017",
            "预处理备注（prep_notes）缺失率 > 60% 的门店，损耗率高于同组 p75",
            {
                "metric": "waste_rate",
                "operator": ">",
                "threshold_ref": "peer.p75",
                "condition_and": {"metric": "prep_notes_missing_rate", "operator": ">", "threshold": 0.60},
            },
            {"root_cause": "missing_prep_guidance", "confidence": 0.68, "action": "强制要求关键食材填写预处理备注"},
            C,
            0.68,
        ),
        (
            "CROSS-018",
            "损耗系数（waste_factor）设置全为 0 的门店，BOM 偏差普遍更大",
            {
                "metric": "bom_variance_pct",
                "operator": ">",
                "threshold": 0.20,
                "condition_and": {"metric": "waste_factor_zero_rate", "operator": "==", "value": 1.0},
            },
            {
                "root_cause": "missing_waste_factor_calibration",
                "confidence": 0.73,
                "action": "根据历史损耗事件自动反推 waste_factor 建议值",
            },
            C,
            0.73,
        ),
        (
            "CROSS-019",
            "同一食材在不同门店 BOM 中单位不一致 → 计量混乱",
            {"metric": "ingredient_unit_inconsistency", "operator": ">", "threshold": 1, "min_store_count": 2},
            {
                "root_cause": "measurement_unit_inconsistency",
                "confidence": 0.90,
                "action": "启动食材计量单位标准化专项，统一为 SI 单位",
            },
            C,
            0.90,
        ),
        (
            "CROSS-020",
            "BOM 成本快照与供应商最新报价偏差 > 15% 的门店占比 > 30%",
            {"metric": "bom_cost_freshness_gap", "operator": ">", "threshold": 0.15, "affected_store_ratio": 0.30},
            {"root_cause": "bom_cost_snapshot_stale", "confidence": 0.77, "action": "触发全品牌 BOM 成本批量更新任务"},
            C,
            0.77,
        ),
        ...
        (
            ...
            {"root_cause": "high_ingredient_cost_ratio", "confidence": 0.74, "action": "对比同组采购单价，触发集采谈判"},
            C,
            0.74,
        ),
        (
            "CROSS-022",
            "同城市同档门店食材成本率差异 > 5 个百分点 → 本地采购议价空间",
            {
                "metric": "cost_ratio_city_spread",
                "operator": ">",
                "threshold": 0.05,
                "peer_filter": {"city": "$store.city", "tier": "$store.tier"},
            },
            {
                "root_cause": "local_procurement_price_gap",
                "confidence": 0.70,
                "action": "低价门店提供供应商联系方式，推动跨店共享采购",
            },
            C,
            0.70,
        ),
        (
            "CROSS-023",
            "连锁门店间同食材采购单价差异 > 10% → 集中采购未到位",
            {"metric": "ingredient_unit_price_variance_pct", "operator": ">", "threshold": 0.10, "min_store_count": 3},
            {
                "root_cause": "centralized_procurement_failure",
                "confidence": 0.82,
                "action": "接入集采平台，统一供应商准入和框架合同",
            },
            C,
            0.82,
        ),
        (
            "CROSS-024",
            "门店规范化成本率持续低于同组 p25 → 建立标杆成本模型",
            {"metric": "cost_ratio", "operator": "<", "threshold_ref": "peer.p25", "window_days": 30, "min_peer_count": 3},
            {"root_cause": "cost_excellence", "confidence": 0.78, "action": "提炼成本控制经验，输出标杆成本模型到知识库"},
            C,
            0.78,
        ),
        (
            "CROSS-025",
            "人力成本率高于同组 p75 且食材成本率正常 → 排班效率问题",
            {
                "metric": "labor_ratio",
                "operator": ">",
                "threshold_ref": "peer.p75",
                "condition_and": {"metric": "cost_ratio", "operator": "<=", "threshold_ref": "peer.p50"},
            },
            {
                "root_cause": "labor_scheduling_inefficiency",
                "confidence": 0.71,
                "action": "对比同组排班方案，分析人时产出差距",
            },
            C,
            0.71,
        ),
        (
            "CROSS-026",
            "淡季成本率高于旺季同组 p50 → 淡季费用管控缺位",
            {"metric": "cost_ratio", "operator": ">", "threshold_ref": "peer.p50", "season": "off_peak"},
            {
                "root_cause": "off_peak_cost_control_gap",
                "confidence": 0.67,
                "action": "制定淡季缩量运营方案，弹性排班 + 减少备货",
            },
            C,
            0.67,
        ),
        (
            "CROSS-027",
            "供应商更换后成本率未下降 → 议价效果不达预期",
            {"metric": "cost_ratio_post_supplier_change", "operator": ">", "threshold_ref": "peer.p50"},
            {
                "root_cause": "supplier_negotiation_ineffective",
                "confidence": 0.66,
                "action": "重新评估供应商选择标准，引入多报价竞标",
            },
            C,
            0.66,
        ),
        (
            "CROSS-028",
            "损耗成本在总食材成本中占比 > 同组 p75 → 损耗治理优先于采购优化",
            {"metric": "waste_cost_ratio_in_total", "operator": ">", "threshold_ref": "peer.p75"},
            {"root_cause": "waste_dominates_cost", "confidence": 0.79, "action": "暂缓采购谈判，将资源集中在损耗控制"},
            C,
            0.79,
        ),
        (
            "CROSS-029",
            "特殊食材（进口/高端）占比高于同组 p90 → 菜品结构性成本风险",
            {"metric": "premium_ingredient_ratio", "operator": ">", "threshold_ref": "peer.p90"},
            {"root_cause": "premium_ingredient_overuse", "confidence": 0.69, "action": "优化菜品结构，平衡高低价食材配比"},
            C,
            0.69,
        ),
        (
            "CROSS-030",
            "同组门店近 90 天平均成本率趋势性上升 → 品类性通货膨胀",
            {
                "metric": "cost_ratio_trend_90d",
                "operator": ">",
                "threshold": 0.0,
                "peer_filter": {"tier": "$store.tier"},
                "min_peer_count": 5,
            ...
            {
                "root_cause": "category_cost_inflation",
                "confidence": 0.84,
                "action": "启动品类替代方案调研，评估菜单调价可行性",
            },
            C,
            0.84,
        ),
        ...
        (
            ...
            {"root_cause": "low_seat_productivity", "confidence": 0.73, "action": "对比同组翻台率与客单价，确定主要瓶颈"},
            C,
            0.73,
        ),
        (
            "CROSS-032",
            "菜单覆盖率低于同组 p25 → 门店菜品开发滞后",
            {"metric": "menu_coverage", "operator": "<", "threshold_ref": "peer.p25", "min_peer_count": 3},
            {
                "root_cause": "menu_development_lag",
                "confidence": 0.68,
                "action": "推送总部新品 BOM 包，要求 30 天内完成门店导入",
            },
            C,
            0.68,
        ),
        (
            "CROSS-033",
            "同组门店中只有该门店缺少某热门菜品 → 菜单漏洞",
            {"metric": "missing_popular_dish_count", "operator": ">", "threshold": 0},
            {"root_cause": "menu_gap_vs_peers", "confidence": 0.76, "action": "补充缺失热门菜品 BOM，安排厨师培训"},
            C,
            0.76,
        ),
        (
            "CROSS-034",
            "新品上线后销量低于同组同新品平均销量 50% → 门店推广执行不足",
            {"metric": "new_dish_sales_vs_peer_avg", "operator": "<", "threshold": 0.50, "dish_age_days_max": 30},
            {
                "root_cause": "new_dish_promotion_underexecution",
                "confidence": 0.71,
                "action": "检查前厅推荐话术执行情况，对标高销量门店",
            },
            C,
            0.71,
        ),
        (
            "CROSS-035",
            "同组门店工单完成率普遍下降 → 平台型系统性问题",
            {
                "metric": "task_completion_rate_trend",
                "operator": "<",
                "threshold": -0.10,
                "peer_ratio_min": 0.60,
                "window_days": 7,
            ...
            {
                "root_cause": "platform_systemic_issue",
                "confidence": 0.88,
                "action": "上报总部运营，排查平台工单系统或共性培训缺口",
            },
            C,
            0.88,
        ),
        (
            "CROSS-036",
            "员工绩效分低于同组 p25 且损耗高于同组 p75 → 人员能力瓶颈",
            {
                "metric": "staff_performance_score",
                "operator": "<",
                "threshold_ref": "peer.p25",
                "condition_and": {"metric": "waste_rate", "operator": ">", "threshold_ref": "peer.p75"},
            },
            {
                "root_cause": "staff_capability_bottleneck",
                "confidence": 0.80,
                "action": "启动人才梯队建设，引进高绩效门店人才",
            },
            C,
            0.80,
        ),
        (
            "CROSS-037",
            "外卖销售占比高于同组 p75 但食材损耗也同步高 → 外卖订单管理问题",
            {
                "metric": "takeout_ratio",
                "operator": ">",
                "threshold_ref": "peer.p75",
                "condition_and": {"metric": "waste_rate", "operator": ">", "threshold_ref": "peer.p50"},
            },
            {
                "root_cause": "takeout_order_waste_correlation",
                "confidence": 0.69,
                "action": "优化外卖备货策略，避免因峰值备料导致损耗",
            },
            C,
            0.69,
        ),
        (
            "CROSS-038",
            "同组最佳实践门店损耗率已低于 $threshold，而该门店仍差距 > 5%",
            {
                "metric": "gap_to_best_practice",
                "operator": ">",
                "threshold": 0.05,
                "reference": "peer.min",
                "min_peer_count": 3,
            },
            {"root_cause": "best_practice_gap", "confidence": 0.77, "action": "安排向最佳实践门店互访学习，引进关键经验"},
            C,
            0.77,
        ),
        (
            "CROSS-039",
            "同城市门店间差旅学习频次为零 → 知识孤岛",
            {"metric": "cross_store_visit_count", "operator": "==", "threshold": 0, "window_days": 90},
            {"root_cause": "knowledge_island", "confidence": 0.65, "action": "制定季度跨店交流计划，强制执行互访机制"},
            C,
            0.65,
        ),
        (
            "CROSS-040",
            "开业超 12 个月门店效率指标仍未达到同组中位数 → 成熟化停滞",
            {"metric": "revenue_per_seat", "operator": "<", "threshold_ref": "peer.p50", "store_age_months_min": 12},
            {"root_cause": "maturity_plateau", "confidence": 0.75, "action": "委派区域运营督导深度驻店，输出改善路线图"},
            C,
            0.75,
        ),
        ...
        (
            "CROSS-041",
            "某门店损耗率改善超同组 p90 → 萃取改善经验，复制到全组",
            {
                "metric": "waste_rate_improvement_30d",
                "operator": ">",
                "threshold_ref": "peer.p90_improvement",
                "min_peer_count": 3,
            },
            {"root_cause": "improvement_leader", "confidence": 0.85, "action": "启动知识萃取流程：采访店长，制作SOP案例视频"},
            C,
            0.85,
        ),
        (
            "CROSS-042",
            "最佳实践门店 BOM 配方被采纳率超 80% → 晋升为品牌标准版",
            {"metric": "bom_adoption_rate_in_peer", "operator": ">", "threshold": 0.80},
            {"root_cause": "bom_best_practice_adoption", "confidence": 0.82, "action": "将该门店 BOM 版本提升为总部标准版本"},
            C,
            0.82,
        ),
        (
            "CROSS-043",
            "某门店成功解决某根因问题后，同组其他门店仍有相同根因 → 知识传播不足",
            {"metric": "root_cause_peer_remaining_count", "operator": ">", "threshold": 2, "same_root_cause": True},
            {
                "root_cause": "knowledge_transfer_gap",
                "confidence": 0.74,
                "action": "将成功案例推送给同根因门店，标注解决方案步骤",
            },
            C,
            0.74,
        ),
        (
            "CROSS-044",
            "知识规则命中率在同组内排名 Top1 → 门店学习能力领先",
            {"metric": "rule_hit_accuracy_rate", "operator": "==", "value": "rank_1_in_peer"},
            {
                "root_cause": "knowledge_absorption_leader",
                "confidence": 0.78,
                "action": "授予屯象OS门店知识贡献认证，开放数据分成权益",
            },
            C,
            0.78,
        ),
        (
            "CROSS-045",
            "同组内某改善措施在 3 家门店验证有效 → 可推全组",
            {"metric": "validated_improvement_store_count", "operator": ">=", "threshold": 3},
            {
                "root_cause": "improvement_ready_for_rollout",
                "confidence": 0.88,
                "action": "发布集团级改善推广令，设置 14 天落地期限",
            },
            C,
            0.88,
        ),
        (
            "CROSS-046",
            "同类门店供应商评分最低 25% 的供应商，该门店仍在使用 → 供应商选择落后",
            {"metric": "low_rated_supplier_usage", "operator": ">", "threshold": 0},
            {
                "root_cause": "poor_supplier_selection",
                "confidence": 0.76,
                "action": "强制替换评分后25%供应商，参考同组优质供应商",
            },
            C,
            0.76,
        ),
        (
            "CROSS-047",
            "行业基准损耗率已更新，门店目标设置仍按旧基准 → 目标过时",
            {"metric": "target_vs_latest_benchmark_gap", "operator": ">", "threshold": 0.05},
            {"root_cause": "stale_performance_target", "confidence": 0.71, "action": "同步更新门店损耗率目标至最新行业基准"},
            C,
            0.71,
        ),
        (
            "CROSS-048",
            "本门店在某食材损耗上被多家同组门店认定为最佳 → 申报行业标杆",
            {"metric": "ingredient_waste_peer_rank", "operator": "==", "value": "rank_1", "min_peer_count": 5},
            {
                "root_cause": "ingredient_management_champion",
                "confidence": 0.83,
                "action": "申报品牌年度食材管理最佳实践，纳入模型市场",
            },
            C,
            0.83,
        ),
        (
            "CROSS-049",
            "同组门店平均损耗率近半年持续优于行业基准 p75 → 组级知识资产积累",
            {
                "metric": "peer_group_avg_waste_rate",
                "operator": "<",
                "threshold_ref": "industry.p75",
                "window_months": 6,
                "min_peer_count": 3,
            },
            {
                "root_cause": "group_knowledge_asset_maturity",
                "confidence": 0.81,
                "action": "将同组经验汇编为行业白皮书，上传模型市场",
            },
            C,
            0.81,
        ),
        (
            "CROSS-050",
            "两门店相似度 > 0.80 但损耗率差距 > 15% → 强对标学习价值",
            {
                "metric": "waste_rate_gap_vs_similar_store",
                "operator": ">",
                "threshold": 0.15,
                "condition_and": {"metric": "similarity_score", "operator": ">", "threshold": 0.80},
            },
            {
                "root_cause": "high_similarity_learning_opportunity",
                "confidence": 0.86,
                "action": "安排 1 对 1 门店配对学习，明确差距清单",
            },
            C,
            0.86,
        ),
...
class KnowledgeRuleService:
    ...
    async def get_by_id(self, rule_id: str) -> Optional[KnowledgeRule]:
        stmt = select(KnowledgeRule).where(KnowledgeRule.id == uuid.UUID(rule_id))
        result = await self.db.execute(stmt)
        return result.scalar_one_or_none()
    ...
    async def activate_rule(self, rule_id: str) -> bool:
        await self.db.execute(
            update(KnowledgeRule).where(KnowledgeRule.id == uuid.UUID(rule_id)).values(status=RuleStatus.ACTIVE)
        )
        return True
    ...
    async def archive_rule(self, rule_id: str, superseded_by: Optional[str] = None) -> bool:
        ...
        await self.db.execute(update(KnowledgeRule).where(KnowledgeRule.id == uuid.UUID(rule_id)).values(**values))
        return True
    ...
    async def get_benchmarks(
        ...
        stmt = select(IndustryBenchmark).where(and_(*conditions)).order_by(IndustryBenchmark.metric_name)
        result = await self.db.execute(stmt)
        return list(result.scalars().all())
            ...
            results.append(
                {
                    ...
                    "status": ("above_median" if pct in ("top_10", "75-90", "50-75") else "below_median"),
                }
            )
        return results
    ...
    async def get_rule_stats(self) -> Dict:
        ...
        active = await self.db.execute(select(func.count(KnowledgeRule.id)).where(KnowledgeRule.status == RuleStatus.ACTIVE))
        by_category = await self.db.execute(
            select(KnowledgeRule.category, func.count(KnowledgeRule.id)).group_by(KnowledgeRule.category)
        )
        return {
