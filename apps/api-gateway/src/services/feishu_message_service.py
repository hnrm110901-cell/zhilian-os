...
class FeishuMessageService:
    ...
    async def get_tenant_access_token(self) -> str:
        ...
        async with httpx.AsyncClient() as client:
            ...
            # 缓存token，提前5分钟过期
            await redis_cache.set(self._cache_key_tenant, tenant_access_token, expire=expire - 300)

            logger.info("飞书tenant_access_token获取成功并已缓存", expire=expire)
            return tenant_access_token

    async def send_text_message(self, receive_id: str, content: str, receive_id_type: str = "open_id") -> Dict[str, Any]:
        """
        发送文本消息
        ...
        try:
            ...
            message_data = {"receive_id": receive_id, "msg_type": "text", "content": json.dumps({"text": content})}
            ...
            async with httpx.AsyncClient() as client:
                ...
                if result.get("code") == 0:
                    logger.info(
                        "飞书文本消息发送成功", receive_id=receive_id, message_id=result.get("data", {}).get("message_id")
                    )
                    return {"success": True, "message_id": result.get("data", {}).get("message_id")}
                else:
                    logger.error("飞书消息发送失败", error=result.get("msg"), code=result.get("code"))
    ...
    async def send_rich_text_message(
        self, receive_id: str, title: str, content: List[List[Dict]], receive_id_type: str = "open_id"
    ) -> Dict[str, Any]:
        """
        发送富文本消息
        ...
        try:
            ...
            rich_text_content = {"zh_cn": {"title": title, "content": content}}

            message_data = {"receive_id": receive_id, "msg_type": "post", "content": json.dumps(rich_text_content)}
            ...
            async with httpx.AsyncClient() as client:
                ...
                if result.get("code") == 0:
                    ...
                    logger.error("飞书富文本消息发送失败", error=result.get("msg"))
    ...
    async def send_card_message(
        self, receive_id: str, card_content: Dict[str, Any], receive_id_type: str = "open_id"
    ) -> Dict[str, Any]:
        """
        发送卡片消息
        ...
        try:
            ...
            message_data = {"receive_id": receive_id, "msg_type": "interactive", "content": json.dumps(card_content)}
            ...
            async with httpx.AsyncClient() as client:
                ...
                if result.get("code") == 0:
                    ...
                    logger.error("飞书卡片消息发送失败", error=result.get("msg"))
                    return {"success": False, "error": result.get("msg")}
    ...
    async def send_notification_card(
        self, receive_id: str, title: str, content: str, url: Optional[str] = None, receive_id_type: str = "open_id"
    ) -> Dict[str, Any]:
        """
        发送通知卡片（简化版）
            ...
            card_content["elements"].append(
                {
                    "tag": "action",
                    "actions": [
                        {"tag": "button", "text": {"tag": "plain_text", "content": "查看详情"}, "type": "primary", "url": url}
                    ],
                }
            )
    ...
    async def send_group_message(self, chat_id: str, content: str, msg_type: str = "text") -> Dict[str, Any]:
        """
        发送群组消息
        ...
        Returns:
            发送结果
        """
        return await self.send_text_message(receive_id=chat_id, content=content, receive_id_type="chat_id")
