...
class AgentConflict(Base):
    ...
    # 仲裁
    arbitration_status = Column(
        SAEnum(ArbitrationStatusEnum, name="arbitration_status_enum"), default=ArbitrationStatusEnum.pending, nullable=False
    )
...
class GlobalOptimizationLog(Base):
    ...
    __table_args__ = (Index("idx_global_opt_store", "store_id", "created_at"),)
...
class AgentCollabSnapshot(Base):
    ...
    __table_args__ = (Index("idx_collab_snap_brand_date", "brand_id", "snapshot_date"),)
