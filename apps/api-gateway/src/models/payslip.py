...
class PayslipRecord(Base, TimestampMixin):
    ...
    __table_args__ = (UniqueConstraint("store_id", "employee_id", "pay_month", name="uq_payslip_month"),)
    ...
    def __repr__(self):
        return f"<PayslipRecord(employee='{self.employee_id}', " f"month='{self.pay_month}', status='{self.push_status}')>"
