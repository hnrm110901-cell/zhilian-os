...
class SocialInsuranceConfig(Base, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("region_code", "effective_year", name="uq_si_region_year"),
    )
...
class EmployeeSocialInsurance(Base, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("employee_id", "effective_year", name="uq_emp_si_year"),
    )
    ...
    def __repr__(self):
        return (
            f"<EmployeeSocialInsurance(employee='{self.employee_id}', "
            f"base={self.personal_base_fen / 100:.0f}yuan)>"
        )
