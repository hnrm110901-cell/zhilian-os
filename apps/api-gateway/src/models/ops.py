...
from sqlalchemy import Column, String, Integer, Float, Boolean, JSON, DateTime, Text, ForeignKey, Index, func
...
class OpsDeviceReading(Base):
    ...
    metric_type = Column(String(50), nullable=False,
                         comment="temperature/power/online_status/tpm/clean_days")
...
class OpsNetworkHealth(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    store_id = Column(String(50), ForeignKey("stores.id"), nullable=False)
    probe_type = Column(String(30), nullable=False,
                        comment="icmp/http/dns/bandwidth/wifi/vpn")
    target = Column(String(200), nullable=False)
    vlan = Column(String(20), nullable=True,
                  comment="vlan10/vlan20/vlan30/vlan40/vlan50/wan")
...
class OpsSysHealthCheck(Base):
    ...
    priority = Column(String(5), nullable=False, default="P2",
                      comment="P0/P1/P2/P3")
    check_method = Column(String(30), nullable=False,
                          comment="api_heartbeat/db_probe/port_check/process_check")
    is_available = Column(Boolean, nullable=False, default=True)
...
class OpsFoodSafetyRecord(Base):
    ...
    record_type = Column(String(30), nullable=False,
                         comment="cold_chain/fridge_power/ice_machine_clean/oil_quality/safety_device")
