...
async def priorities(
    ...
    if priority not in _VALID_PRIORITIES:
        raise HTTPException(status_code=400, detail=f"priority 必须是 {_VALID_PRIORITIES} 之一")
    items = await get_action_priorities(db, store_id, period, priority=priority, limit=limit)
    return {
        "store_id": store_id,
        "period": period,
        "priority": priority,
...
async def list_scores(
    ...
    if health_tier and health_tier not in _VALID_TIERS:
        raise HTTPException(status_code=400, detail=f"health_tier 必须是 {_VALID_TIERS} 之一")
    recs = await get_health_scores(db, store_id, period, health_tier=health_tier, limit=limit)
    return {"store_id": store_id, "period": period, "count": len(recs), "scores": recs}
...
async def meta_scoring():
    return {
        "dimensions": {
            "profitability": {
                "max": 25,
                "basis": "GPM + FCR vs 门店同期均值 (各 12.5 分)",
            },
            "growth": {
                "max": 25,
