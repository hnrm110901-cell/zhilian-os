...
async def list_records(
    ...
    records = await get_lifecycle_records(db, store_id, period, phase=phase, limit=limit)
    return {"store_id": store_id, "period": period, "count": len(records), "records": records}
...
async def transition_alerts(
    ...
    return {"store_id": store_id, "period": period, "count": len(alerts), "transitions": alerts}
...
async def meta_phases():
    return {
        ...
        "phase_actions": {p: {"label": v["action_label"], "action": v["recommended_action"]} for p, v in PHASE_ACTION.items()},
