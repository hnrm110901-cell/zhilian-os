...
async def get_tax_calculations(
    ...
    if not results:
        return {"store_id": store_id, "period": period, "message": "暂无税务计算结果，请先调用 compute", "results": []}
...
async def compute_cashflow(
    ...
    """触发现金流预测（幂等，当日覆盖）"""
    result = await compute_cashflow_forecast(db, store_id, opening_balance=opening_balance, force=force)
...
async def get_cashflow_forecast(
    ...
    rows = (
        ...
        LIMIT :days
    """),
            {"sid": store_id, "gen": gen_str, "days": days},
        )
    ).fetchall()
...
async def get_agent_actions(
    ...
    if status and level:
        rows = (
            ...
            LIMIT :limit
        """),
                {"sid": store_id, "status": status, "level": level, "limit": limit},
            )
        ).fetchall()
    elif status:
        rows = (
            ...
            LIMIT :limit
        """),
                {"sid": store_id, "status": status, "limit": limit},
            )
        ).fetchall()
    elif level:
        rows = (
            ...
            LIMIT :limit
        """),
                {"sid": store_id, "level": level, "limit": limit},
            )
        ).fetchall()
    else:
        rows = (
