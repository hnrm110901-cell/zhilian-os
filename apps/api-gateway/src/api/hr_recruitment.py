...
async def list_jobs(
    ...
    result = await db.execute(select(JobPosting).where(and_(*conditions)).order_by(JobPosting.created_at.desc()))
...
async def list_candidates(
    ...
    result = await db.execute(select(Candidate).where(and_(*conditions)).order_by(Candidate.created_at.desc()))
...
async def get_recruitment_funnel(
    ...
    return {
        ...
        "funnel": funnel,
        "conversion_rate": (round(funnel.get("hired", 0) / max(funnel.get("new", 1), 1) * 100, 1)),
    }
