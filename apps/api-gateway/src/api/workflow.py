...
class StartWorkflowIn(BaseModel):
    plan_date: Optional[str] = Field(None, description="规划日期（默认=明天），格式 YYYY-MM-DD")
    store_config: Optional[Dict[str, Any]] = Field(None, description="门店个性化截止时间，如 {procurement_deadline: '17:45'}")


class SubmitDecisionIn(BaseModel):
    content: Dict[str, Any] = Field(..., description="决策内容（各阶段格式不同，见 API 文档）")
    mode: str = Field("manual", description="提交模式: fast / precise / manual")
...
async def get_today_summary(
    ...
    # 今日所有工作流
    wf_stmt = select(DailyWorkflow).where(DailyWorkflow.trigger_date == today)
    ...
    timing = TimingService(db)
    for wf in workflows:
        phases = (await db.execute(select(WorkflowPhase).where(WorkflowPhase.workflow_id == wf.id))).scalars().all()

        all_locked = all(p.status == PhaseStatus.LOCKED.value for p in phases)
        any_overdue = any(
            timing.is_overdue(p) for p in phases if p.status in (PhaseStatus.RUNNING.value, PhaseStatus.REVIEWING.value)
        )
        ...
        for p in phases:
            if p.phase_name in phase_dist:
                bucket = (
                    "locked"
                    if p.status == PhaseStatus.LOCKED.value
                    else "running" if p.status in (PhaseStatus.RUNNING.value, PhaseStatus.REVIEWING.value) else "pending"
                )
...
async def _get_phase_and_wf(phase_id: _uuid.UUID, db: AsyncSession) -> tuple:
