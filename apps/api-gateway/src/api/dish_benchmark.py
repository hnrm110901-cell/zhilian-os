...
async def store_benchmark(
    ...
    if fcr_tier and fcr_tier not in TIERS:
        raise HTTPException(status_code=400, detail=f"fcr_tier 必须是 {TIERS} 之一")
    records = await get_dish_benchmark(db, store_id, period, fcr_tier=fcr_tier, limit=limit)
...
async def dish_cross_store(
    ...
    return {"dish_name": dish_name, "period": period, "store_count": len(detail), "stores": detail}
