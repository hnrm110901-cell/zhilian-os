...
async def list_alerts(
    ...
    if severity and severity not in SEVERITIES:
        raise HTTPException(status_code=400, detail=f"severity 必须是 {SEVERITIES} 之一")
    if status and status not in ("open", "resolved", "suppressed"):
        raise HTTPException(status_code=400, detail="status 必须是 open/resolved/suppressed")
    alerts = await get_dish_cost_alerts(db, store_id, period, severity=severity, status=status, limit=limit)
    return {
        "store_id": store_id,
        "period": period,
        "count": len(alerts),
...
async def meta_types():
    return {
        "alert_types": [{"key": at, "label": ALERT_LABELS[at]} for at in ALERT_TYPES],
