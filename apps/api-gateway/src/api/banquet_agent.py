...
async def get_period_comparison(
    ...
    async def _kpi_for(start: date_type, end: date_type):
        ...
        return {"revenue_yuan": rev, "order_count": orders, "lead_count": leads,
                "conversion_rate_pct": conv, "avg_order_yuan": avg_o}
...
async def get_hall_peak_booking(
    ...
    for hid, hall in hall_map.items():
        ...
        wd_data = [
            {"weekday": i, "name": WEEKDAY_NAMES[i], "count": stats["weekday"].get(i, 0)}
            for i in range(7)
        ]
...
async def get_task_completion_trend(
    ...
    res = await db.execute(
        select(ExecutionTask).join(
            BanquetOrder, BanquetOrder.id == ExecutionTask.banquet_order_id
        ).where(
            BanquetOrder.store_id == store_id,
...
async def get_quote_conversion(
    ...
    trend = [
        {"month": mo, "quoted": d["quoted"], "won": d["won"],
...
async def get_lead_channel_weekly(
    ...
    series = [
        {"week": wk, **{ch: data[wk].get(ch, 0) for ch in channels}}
...
async def get_cancellation_analysis(
    ...
    if not orders:
        return {
            "store_id": store_id, "months": months,
            "total": 0,
            "total_cancelled": 0, "cancel_rate_pct": None,
            "revenue_lost_yuan": 0.0,
            "total_lost_yuan": 0.0, "by_type": [], "by_banquet_type": [], "by_month": [],
        }
    ...
    type_rows = [
        {"banquet_type": k, "count": v,
         "pct": round(v / len(orders) * 100, 1)}
        for k, v in sorted(by_type.items(), key=lambda x: -x[1])
    ]
    month_rows = [
        {"month": mo, "count": d["count"], "lost_yuan": round(d["lost_yuan"], 2)}
...
async def get_package_upgrade_rate(
    ...
    packages = pkg_res.scalars().all()
    if not packages:
        return {"store_id": store_id, "packages": [], "upgrade_count": 0,
                "avg_upgrade_yuan": None, "upgrade_rate_pct": None}
    ...
    avg_up = round(
        sum(u["package_price_yuan"] for u in upgrade_orders) / len(upgrade_orders), 2
    ) if upgrade_orders else None
...
async def get_revenue_per_table(
    ...
    if not orders:
        return {"store_id": store_id, "months": months,
                "overall_avg_yuan": None, "by_type": [], "by_month": []}
...
async def get_cross_type_spending(
    ...
    cross_customers = [
        cid for cid, types in cust_types.items() if len(types) >= 2
    ]
    ...
    pairs = [
        {"pair": p, "customer_count": c}
        for p, c in sorted(pair_counts.items(), key=lambda x: -x[1])
    ]

    avg_cross_spend = (
        round(sum(cust_spend[c] for c in cross_customers) / len(cross_customers), 2)
        if cross_customers else None
    )
...
async def get_waitlist_conversion(
    ...
    for l in leads:
        ...
        if stage in ("waiting_decision", "deposit_pending", "signed"):
            ...
            if stage == "signed":
                converted.append(l)
                created = l.created_at if isinstance(l.created_at, datetime) else datetime.combine(l.created_at, datetime.min.time())
...
async def get_refund_rate(
    ...
    type_rows = [
        {"banquet_type": k, "count": v,
         "pct": round(v / len(refund_orders) * 100, 1) if refund_orders else 0.0}
        for k, v in sorted(by_type.items(), key=lambda x: -x[1])
    ]
...
async def get_lead_response_time(
    ...
    buckets_out = [
        {"label": label, "count": cnt,
         "pct": round(cnt / responded * 100, 1) if responded else 0.0}
        for label, cnt in bucket_counts.items()
    ]
...
async def get_bundle_performance(
    ...
    for pid, orders_list in pkg_orders.items():
        ...
        pkg_stats.append({
            ...
            "gross_margin_pct": round(
                (pkg.suggested_price_fen - pkg.cost_fen) / pkg.suggested_price_fen * 100, 1
            ) if pkg.suggested_price_fen else None,
        })
...
async def get_review_sentiment_trend(
    ...
    res = await db.execute(
        select(BanquetOrderReview).join(
            BanquetOrder, BanquetOrder.id == BanquetOrderReview.banquet_order_id
        ).where(
            BanquetOrder.store_id == store_id,
    ...
    trend = [
        {
            ...
            "neutral":  d["neu"],
            "negative": d["neg"],
            "avg_ai_score": round(sum(d["ai_scores"]) / len(d["ai_scores"]), 1)
                            if d["ai_scores"] else None,
        }
        for mo, d in sorted(monthly.items())
    ]
...
async def get_vip_churn_early_warning(
    ...
    for c in customers:
        ...
        months_inactive = round(
            (date_type.today() - last_order.banquet_date).days / 30, 1
        )
...
async def get_capacity_revenue_yield(
    ...
    avg_yield = round(
        sum(h["utilization_pct"] for h in hall_yields) / len(hall_yields), 1
    ) if hall_yields else None
...
async def get_hall_occupancy_forecast(
    ...
    overall = round(
        sum(h["booked_days"] for h in hall_forecasts) /
        (len(hall_forecasts) * days_ahead) * 100, 1
    ) if hall_forecasts else None
...
async def get_staff_exception_rate(
    ...
    res = await db.execute(
        select(ExecutionException).join(
            BanquetOrder, BanquetOrder.id == ExecutionException.banquet_order_id
        ).where(
            BanquetOrder.store_id == store_id,
...
async def get_customer_lifetime_value(
    ...
    customers = cust_res.scalars().all()
    if not customers:
        return {"store_id": store_id, "total_customers": 0,
                "avg_ltv_yuan": None, "percentiles": {}, "top": []}

    ltvs = [(c.id, round((c.total_banquet_amount_fen or 0) / 100, 2),
             c.total_banquet_count or 0) for c in customers]
    ...
    top_customers = [
        {"customer_id": cid, "ltv_yuan": ltv, "banquet_count": cnt}
        for cid, ltv, cnt in ltvs[:top_n]
    ]
...
async def get_payment_method_distribution(
    ...
    cutoff = date_type.today() - timedelta(days=months * 30)
    res = await db.execute(
        select(BanquetPaymentRecord).join(
            BanquetOrder, BanquetOrder.id == BanquetPaymentRecord.banquet_order_id
        ).where(
            BanquetOrder.store_id == store_id,
...
async def get_banquet_size_revenue_correlation(
    ...
    if not orders:
        return {"store_id": store_id, "months": months,
                "total_orders": 0, "size_groups": [], "inflection_point": None}
...
async def get_follow_up_effectiveness(
    ...
    for lead in leads:
        ...
        bucket = (
            "0次" if count == 0 else
            "1次" if count == 1 else
            "2-3次" if count <= 3 else
            "4-6次" if count <= 6 else
            "7次以上"
        )
    ...
    rows = [
        {
            ...
            "win_rate_pct": round(
                followup_groups[b]["won"] / followup_groups[b]["total"] * 100, 1
            ) if followup_groups[b]["total"] else 0.0,
        }
        for b in ORDER if followup_groups[b]["total"] > 0
    ]
...
async def get_repeat_customer_rate(
    ...
    res = await db.execute(
        select(BanquetOrder).where(
            ...
            BanquetOrder.banquet_date >= cutoff,
            BanquetOrder.order_status.in_([
                OrderStatusEnum.CONFIRMED, OrderStatusEnum.COMPLETED,
            ]),
        )
    )
    orders = res.scalars().all()
    ...
    for o in orders:
        ...
        if o.package_id:
            res_pkg = await db.execute(
                select(MenuPackage).where(MenuPackage.id == o.package_id)
            )
...
async def get_lead_dropout_stage(
    ...
    if not leads:
        return {
            "store_id": store_id, "months": months,
            "total_leads": 0, "stages": [], "max_dropout_stage": None,
        }
...
async def get_upsell_rate(
    ...
    if not orders:
        return {
            "store_id": store_id, "months": months,
            "total_pkg_orders": 0, "upsell_count": 0,
            "upsell_rate_pct": None, "avg_upsell_yuan": None,
        }
...
async def get_multi_hall_booking_rate(
    ...
    if not orders:
        return {
            "store_id": store_id, "months": months,
            "total_orders": 0, "multi_hall_count": 0,
...
async def get_referral_rate(
    ...
    if not leads:
        return {
            "store_id": store_id, "months": months,
            "total_leads": 0, "referral_count": 0,
    ...
    def _win_rate(lst):
        if not lst:
            return None
        won = sum(1 for l in lst if (
            l.current_stage.value if hasattr(l.current_stage, "value") else str(l.current_stage or "")
        ) == "signed")
...
async def get_quote_turnaround_time(
    ...
    if not leads:
        return {
            "store_id": store_id, "months": months,
            "total_leads": 0, "avg_days": None,
            "median_days": None, "buckets": [],
        }
    ...
    if not days_list:
        return {
            "store_id": store_id, "months": months,
            "total_leads": len(leads), "avg_days": None,
            "median_days": None, "buckets": [],
        }

    BUCKETS = [("当天", 0, 0), ("1-2天", 1, 2), ("3-7天", 3, 7),
               ("8-14天", 8, 14), ("15天以上", 15, 9999)]
    ...
    buckets = [
        {"bucket": lbl, "count": bucket_counts[lbl],
         "pct": round(bucket_counts[lbl] / total * 100, 1)}
...
async def get_deposit_to_full_payment_days(
    ...
    if not orders:
        return {
            "store_id": store_id, "months": months,
            "total_fully_paid": 0, "avg_days": None,
            "buckets": [],
        }
    ...
    if not days_list:
        ...
        for d in days_list:
            ...
            elif d <= 30:    bc["8-30天"] += 1
            elif d <= 90:    bc["31-90天"] += 1
            else:            bc["90天以上"] += 1
        total = len(days_list)
        buckets = [
...
async def get_hall_booking_gap(
    ...
    return {
        "store_id": store_id, "months": months,
        "halls": hall_gaps, "overall_avg_gap_days": overall,
    }
...
async def get_contract_signed_rate(
    ...
    if not orders:
        return {
            "store_id": store_id, "months": months,
            "total_orders": 0, "with_contract": 0,
...
async def get_staff_task_overdue_rate(
    ...
    if not tasks:
        return {
            ...
            "overall_overdue_rate_pct": None, "by_staff": [],
        }
...
async def get_monthly_new_vs_repeat(
    ...
    monthly: dict = defaultdict(lambda: {
        "new_orders": 0, "new_rev_fen": 0,
        "repeat_orders": 0, "repeat_rev_fen": 0,
    })
    for o in sorted_orders:
        bd = o.banquet_date
...
async def get_lead_response_speed(
    ...
    if not leads:
        return {
            "store_id": store_id, "months": months,
            "total_leads": 0, "responded_leads": 0,
            "avg_response_hours": None, "fast_response_pct": None,
        }
    ...
    if not hours_list:
        return {
            "store_id": store_id, "months": months,
            "total_leads": len(leads), "responded_leads": 0,
            "avg_response_hours": None, "fast_response_pct": None,
        }
...
async def get_order_amendment_rate(
    ...
    if not orders:
        return {
            "store_id": store_id, "months": months,
            "total_orders": 0, "amended_orders": 0,
...
async def get_vip_upgrade_trend(
    ...
    if not customers:
        return {
            "store_id": store_id, "months": months,
            "total_customers": 0, "by_level": [], "avg_vip_level": None,
        }
...
async def get_banquet_type_profitability(
    ...
    if not orders:
        return {
            "store_id": store_id, "months": months,
            "types": [], "most_profitable_type": None,
        }
...
async def get_hall_peak_season_rate(
    ...
    if not halls:
        return {
            "store_id": store_id, "months": months,
            "halls": [], "peak_total": 0, "offpeak_total": 0,
            "peak_ratio": None,
        }
    ...
    for hall in halls:
        ...
        hall_stats.append({
            "hall_id": hall.id, "hall_name": hall.name,
            "peak_bookings": peak, "offpeak_bookings": offpeak,
        })
...
async def get_package_attach_rate(
    ...
    if not orders:
        return {
            "store_id": store_id, "months": months,
            "total_orders": 0, "with_package": 0,
            "attach_rate_pct": None, "top_packages": [],
        }
...
async def get_customer_reactivation_rate(
    ...
    if not all_orders:
        return {
            "store_id": store_id, "months": months,
            "reactivated_count": 0, "reactivation_rate_pct": None,
        }
...
async def get_event_execution_score(
    ...
    if not orders:
        return {
            "store_id": store_id, "months": months,
            "total_events": 0, "avg_execution_score": None,
            "events": [],
        }
    ...
    for o in orders:
        ...
        done_tasks = sum(1 for t in tasks if t.task_status in (
            TaskStatusEnum.DONE, TaskStatusEnum.VERIFIED, TaskStatusEnum.CLOSED
        ))
...
async def get_cross_sell_rate(
    ...
    if not orders:
        return {
            "store_id": store_id, "months": months,
            "total_customers": 0, "cross_sell_customers": 0,
...
async def get_banquet_size_trend(
    ...
    if not orders:
        return {
            "store_id": store_id, "months": months,
            "overall_avg_tables": None, "monthly": [],
        }
...
async def get_lead_budget_accuracy(
    ...
    if not signed_leads:
        return {
            "store_id": store_id, "months": months,
            "total_signed": 0, "avg_budget_yuan": None,
    ...
    for lead in signed_leads:
        res2 = await db.execute(
            select(BanquetOrder).where(
                BanquetOrder.store_id == store_id,
                BanquetOrder.customer_id == lead.customer_id if hasattr(lead, "customer_id") else BanquetOrder.contact_name == lead.contact_name,
                BanquetOrder.order_status.in_([
                    OrderStatusEnum.CONFIRMED, OrderStatusEnum.COMPLETED,
                ]),
            )
...
async def get_payment_overdue_aging(
    ...
    overdue = [
        o for o in orders
        if (o.total_amount_fen or 0) > (o.paid_fen or 0)
        and o.banquet_date and o.banquet_date < today
    ]

    if not overdue:
        return {
            "store_id": store_id,
            "total_overdue": 0, "total_overdue_yuan": 0.0,
            "buckets": [],
        }
    ...
    for o in overdue:
        ...
        outstanding = (o.total_amount_fen or 0) - (o.paid_fen or 0)
        if days <= 30:       label = "0-30天"
        elif days <= 90:     label = "31-90天"
        elif days <= 180:    label = "91-180天"
        elif days <= 365:    label = "181-365天"
        else:                label = "1年以上"
    ...
    bucket_list = [
        ...
        for lbl in ORDER if buckets[lbl]["count"] > 0
    ]
...
async def get_staff_coverage_rate(
    ...
    if not tasks:
        return {
            "store_id": store_id, "months": months,
            "total_staff": 0, "active_staff": 0,
            "coverage_rate_pct": None, "by_staff": [],
        }
...
async def get_early_checkin_rate(
    ...
    if not done_tasks:
        return {
            "store_id": store_id, "months": months,
            "total_completed": 0, "early_count": 0,
            "early_rate_pct": None,
        }
...
async def get_loyalty_points_redemption_rate(
    ...
    if not customers:
        return {
            "store_id": store_id, "months": months,
            "total_customers": 0, "redemption_customers": 0,
...
async def get_menu_upgrade_rate(
    ...
    if not orders:
        return {
            "store_id": store_id, "months": months,
            "total_pkg_orders": 0, "upgrade_count": 0,
...
async def get_hall_double_booking_risk(
    ...
    if not halls:
        return {
            "store_id": store_id, "months": months,
            "halls": [], "total_conflicts": 0,
        }

    conflicts = []
    for hall in halls:
        res_b = await db.execute(
            select(BanquetHallBooking).where(BanquetHallBooking.hall_id == hall.id)
        )
        ...
        if hall_conflicts:
            conflicts.append({
                ...
                "hall_name": hall.name,
                "conflict_dates": [
                    {"date": d, "order_count": len(ids), "order_ids": ids}
                    for d, ids in hall_conflicts.items()
                ],
            })
...
async def get_post_event_followup_rate(
    ...
    if not orders:
        return {
            ...
            "total_completed": 0, "followup_count": 0,
    ...
    for o in orders:
        ...
        res_f = await db.execute(
            select(LeadFollowupRecord).join(
                BanquetLead, BanquetLead.id == LeadFollowupRecord.lead_id
            ).where(
...
async def get_staff_utilization_heatmap(
    ...
    for uid, stats in sorted(staff_map.items(), key=lambda x: -x[1]["total"]):
        ...
        staff_list.append({
            ...
            "by_weekday": [
                {"weekday": weekday_names[i], "count": stats["by_weekday"][i]}
                for i in range(7)
            ],
        })
...
async def get_deposit_refund_rate(
    ...
    if not cancelled:
        return {
            "store_id": store_id, "months": months,
            "total_cancelled": 0, "deposit_refund_count": 0,
...
async def get_hall_maintenance_downtime(
    ...
    """厅房维护停用率 — 停用厅房占总厅房的比例"""
    res = await db.execute(
        select(BanquetHall).where(BanquetHall.store_id == store_id)
    )
    halls = res.scalars().all()
    ...
    if not halls:
        return {
            "store_id": store_id,
            "total_halls": 0, "inactive_halls": 0,
            "downtime_rate_pct": None, "halls": [],
        }
    ...
    hall_list = [
        {"hall_id": h.id, "name": h.name, "is_active": h.is_active}
        for h in halls
    ]
...
async def get_customer_complaint_rate(
    ...
    if not orders:
        return {
            "store_id": store_id, "months": months,
            "total_completed": 0, "complaint_count": 0,
...
async def get_table_per_staff_ratio(
    ...
    for o in orders:
        ...
        res_t = await db.execute(
            select(ExecutionTask).where(ExecutionTask.banquet_order_id == o.id)
        )
        tasks = res_t.scalars().all()
        total_tasks += len(tasks)
        for t in tasks:
...
async def get_seasonal_revenue_index(
    ...
    if not orders:
        return {
            "store_id": store_id, "months": months,
            "monthly": [], "peak_month": None, "trough_month": None,
        }
...
async def get_staff_overtime_rate(
    ...
    if not tasks:
        return {
            "store_id": store_id, "months": months,
            "total_completed": 0, "overtime_count": 0,
            "overtime_rate_pct": None,
        }

    overtime = [
        t for t in tasks
        if t.completed_at and t.due_time and t.completed_at > t.due_time
    ]
    total = len(tasks)
...
async def get_package_revenue_contribution(
    ...
    if not orders:
        return {
            "store_id": store_id, "months": months,
            "total_orders": 0,
            "pkg_orders": 0, "pkg_revenue_yuan": None,
...
async def get_customer_churn_risk(
    ...
    if not customers:
        return {
            "store_id": store_id, "inactive_months": inactive_months,
            "total_customers": 0, "at_risk_count": 0,
            "churn_risk_pct": None,
        }
...
async def get_hall_revenue_per_sqm(
    ...
    for hall in halls:
        ...
        for bk in bookings:
            res_o = await db.execute(
                select(BanquetOrder).where(BanquetOrder.id == bk.banquet_order_id)
            )
            o = res_o.scalars().first()
            if o:
...
async def get_lead_nurture_effectiveness(
    ...
    for lead in leads:
        res_f = await db.execute(
            select(LeadFollowupRecord).where(LeadFollowupRecord.lead_id == lead.id)
        )
...
async def get_task_completion_speed(
    ...
    if not hours_list:
        return {
            ...
            "avg_hours": None, "median_hours": None, "fast_pct": None,
        }
...
async def get_event_coordinator_performance(
    ...
    for o in orders:
        ...
        avg_rating = (
            sum(r.customer_rating for r in reviews if r.customer_rating) / len(reviews)
...
async def get_hall_booking_lead_time(
    ...
    distribution = [
        ...
        for k in order_keys if counter.get(k, 0) > 0
    ]
    return {
        "total_orders": len(orders),
...
async def get_lead_response_time(
    ...
    for l in leads:
        followup_res = await db.execute(
            select(LeadFollowupRecord).where(
                LeadFollowupRecord.lead_id == l.id
            ).order_by(LeadFollowupRecord.created_at.asc()).limit(1)
        )
...
async def get_customer_satisfaction_score(
    ...
    reviews = res.scalars().all()
    if not reviews:
        return {"total_reviews": 0, "avg_rating": None, "avg_ai_score": None, "nps_estimate": None, "overall_score": None, "monthly": [], "by_month": []}
    ...
    monthly_result = [
        {"month": k, "avg_score": round(sum(v) / len(v), 1), "count": len(v)}
...
async def get_order_size_distribution(
    ...
    def _bucket(tables: int) -> str:
        if tables <= 5:   return "1-5桌"
        if tables <= 10:  return "6-10桌"
        if tables <= 20:  return "11-20桌"
        if tables <= 30:  return "21-30桌"
        return "30桌+"
    ...
    distribution = [
        ...
        for k in order_keys if bucket_data[k]["count"] > 0
    ]
    buckets = [
        {
            "label": item["bucket"],
...
async def get_lead_age_distribution(
    ...
    distribution = [
        {"bucket": k, "count": counter.get(k, 0),
         "pct": round(counter.get(k, 0) / len(age_days) * 100, 1)}
        for k in order_keys if counter.get(k, 0) > 0
    ]
...
async def get_hall_revenue_seasonality(
    ...
    result = sorted(
        [{"month": m, "revenue_yuan": round(v, 2),
          "seasonal_index": round(v / avg, 3) if avg else None}
...
async def get_banquet_cancellation_lead_time(
    ...
    distribution = [
        {"bucket": k, "count": counter.get(k, 0),
         "pct": round(counter.get(k, 0) / len(cancel_days) * 100, 1)}
        for k in order_keys if counter.get(k, 0) > 0
    ]
    return {
        "total_cancelled":       len(orders),
...
async def get_deposit_ratio_analysis(
    ...
    distribution = [
        {"bucket": k, "count": v, "pct": round(v / len(ratios) * 100, 1)}
...
async def get_banquet_no_show_rate(
    ...
    no_show = sum(
        1 for o in orders if o.order_status == OrderStatusEnum.CONFIRMED
    )
    return {
...
async def get_quote_revision_count(
    ...
    cutoff = date_type.today() - timedelta(days=months * 30)
    res = await db.execute(
        select(BanquetQuote).join(
            BanquetLead, BanquetLead.id == BanquetQuote.lead_id
        ).where(
            BanquetLead.store_id == store_id,
...
async def get_hall_peak_booking_slots(
    ...
    cutoff = date_type.today() - timedelta(days=months * 30)
    res = await db.execute(
        select(BanquetHallBooking).join(
            BanquetHall, BanquetHall.id == BanquetHallBooking.hall_id
        ).where(
            BanquetHall.store_id == store_id,
            BanquetHallBooking.slot_date >= cutoff,
        )
...
async def get_customer_acquisition_cost(
    ...
    for l in leads:
        channel_data[l.source_channel].append(
            (l.expected_budget_fen or 0) / 100
        )
    channels = [
        {
            "channel":          ch,
...
async def get_lead_touchpoint_count(
    ...
    for l in leads:
        fu_res = await db.execute(
            select(LeadFollowupRecord).where(LeadFollowupRecord.lead_id == l.id)
        )
...
async def get_staff_specialization_index(
    ...
    for t in tasks:
        ord_res = await db.execute(
            select(BanquetOrder).where(BanquetOrder.id == t.banquet_order_id)
        )
        order = ord_res.scalars().first()
        if order:
...
async def get_lead_budget_accuracy(
    ...
    for l in leads:
        ...
        ord_res = await db.execute(
            ...
            ).order_by(BanquetOrder.created_at.desc()).limit(1)
        )
        order = ord_res.scalars().first()
...
async def get_banquet_addon_revenue(
    ...
    orders = res.scalars().all()
    if not orders:
        return {"total_pkg_orders": 0, "addon_orders": 0,
                "total_addon_yuan": None, "avg_addon_yuan": None}
    addon_amounts = []
    for o in orders:
        pkg_res = await db.execute(
            select(MenuPackage).where(MenuPackage.id == o.package_id)
...
async def get_staff_task_overdue_rate(
    ...
    overdue = sum(
        1 for t in tasks
        if isinstance(getattr(t, "completed_at", None), datetime) and isinstance(getattr(t, "due_time", None), datetime) and t.completed_at > t.due_time
    )
    return {
        "total_completed": len(tasks),
...
async def get_vip_upgrade_rate(
    ...
    by_level = [
        {"level": k, "count": v, "pct": round(v / len(customers) * 100, 1)}
...
async def get_hall_double_booking_risk(
    ...
    conflicts = [
        {"hall_id": k[0], "date": k[1], "slot": k[2], "count": v}
...
async def get_lead_source_roi(
    ...
    if converted_order_ids:
        order_res = await db.execute(
            select(BanquetOrder).where(BanquetOrder.id.in_(converted_order_ids))
        )
        revenue_by_order_id = {
            o.id: round((getattr(o, "total_amount_fen", 0) or 0) / 100, 2)
    ...
    for ch, data in channel_map.items():
        ...
        revenue_yuan = round(sum(revenue_by_order_id.get(getattr(l, "converted_order_id", None), 0.0) for l in channel_leads), 2)
        channels.append({
            "channel":    ch,
            "total":      data["total"],
            "won":        data["won"],
...
async def get_banquet_repeat_venue_rate(
    ...
    repeat_halls = [
        {"hall_id": hid, "booking_count": cnt}
        for hid, cnt in sorted(repeat.items(), key=lambda x: x[1], reverse=True)
    ]
    return {
        "total_bookings":    len(bookings),
...
async def get_menu_upgrade_rate(
    ...
    if not orders:
        return {"total_pkg_orders": 0, "upgrade_count": 0, "upgraded_count": 0, "upgrade_rate_pct": None, "avg_upgrade_yuan": None}
    upgraded = 0
    upgrade_diffs = []
    for o in orders:
        res_pkg = await db.execute(
...
async def get_hall_revenue_per_sqm(
    ...
    for hall in halls:
        ...
        for oid in order_ids:
            res_o = await db.execute(
                select(BanquetOrder).where(BanquetOrder.id == oid)
            )
            o = res_o.scalars().first()
            if o:
...
async def get_staff_rating_by_order(
    ...
    for o in orders:
        ...
        res_r = await db.execute(
            select(BanquetOrderReview).where(BanquetOrderReview.banquet_order_id == o.id)
        )
        review = res_r.scalars().first()
        if review and review.customer_rating:
    ...
    staff = [
        ...
        for name, data in sorted(staff_map.items(), key=lambda x: -(sum(x[1]["ratings"]) / len(x[1]["ratings"]) if x[1]["ratings"] else 0))
    ]
...
async def get_banquet_advance_booking_rate(
    ...
    distribution = [
        {"bucket": label, "count": sum(1 for d in advance_days if lo <= d <= hi)}
        for label, lo, hi in buckets
    ]
    return {
        "total_orders":    len(orders),
...
async def get_lead_lost_reason_analysis(
    ...
    total = len(leads)
    by_channel = [
        {"channel": ch, "count": cnt, "pct": round(cnt / total * 100, 1)}
...
async def get_customer_age_segments(
    ...
    for first in cust_first.values():
        ...
        if days <= 90:
            ...
            seg_cnt["老客户(>365天)"] += 1
    segments = [{"segment": k, "count": v, "pct": round(v / len(cust_first) * 100, 1)}
                for k, v in seg_cnt.items()]
    return {
...
async def get_deposit_collection_rate(
    ...
    avg_dep  = round(sum(o.paid_fen / o.total_amount_fen for o in with_dep if o.total_amount_fen) / len(with_dep) * 100, 1) if with_dep else None
    return {
        "total_confirmed":           len(orders),
...
async def get_slot_revenue_analysis(
    ...
    res_o = await db.execute(
        select(BanquetOrder).where(BanquetOrder.id.in_(order_ids))
    )
...
async def get_lead_source_monthly_trend(
    ...
    for l in leads:
        ...
        ch_total[ch]        += 1
    monthly = [
        {"month": m, "channels": dict(month_ch[m])}
        for m in sorted(month_ch)
    ]
    return {
        "total_leads": len(leads),
        "monthly":     monthly,
...
async def get_order_size_distribution(
    ...
    def _bucket(n: int) -> str:
        if n <= 5:   return "1-5桌"
        if n <= 10:  return "6-10桌"
        if n <= 20:  return "11-20桌"
        if n <= 30:  return "21-30桌"
        return "30桌以上"
    ...
    distribution = [
        ...
        for b in bucket_order if cnt.get(b, 0) > 0
    ]
    buckets = [
        {"label": item["bucket"], "count": item["count"], "pct": item["pct"]}
        for item in distribution
    ]
    avg_tables = round(sum(o.table_count for o in orders) / len(orders), 1)
    return {
...
async def get_package_adoption_rate(
    ...
    orders = res.scalars().all()
    if not orders:
        return {"total_orders": 0, "package_count": 0, "adoption_rate_pct": None,
                "pkg_avg_yuan": None, "no_pkg_avg_yuan": None}
    pkg_orders  = [o for o in orders if o.package_id]
...
async def get_hall_turnover_rate(
    ...
    if not hall_rows:
        return {"total_bookings": 0, "hall_count": 0, "avg_turnover_per_month": None, "halls": [], "overall_turnover_rate": None}
    result_halls = []
    ...
    overall = round(sum((item["turnover_rate"] or 0.0) for item in result_halls) / len(result_halls), 3) if result_halls else None
    return {"total_bookings": total_bookings, "hall_count": hall_count, "avg_turnover_per_month": avg_turnover_per_month, "halls": result_halls, "overall_turnover_rate": overall}
...
async def get_exception_type_breakdown(
    ...
    total = len(exceptions)
    by_type = [
        {"type": t, "count": cnt, "pct": round(cnt / total * 100, 1)}
...
async def get_staff_review_avg(
    ...
    for uid, order_ids in staff_orders.items():
        for oid in order_ids:
            res_r = await db.execute(
                select(BanquetOrderReview).where(
                    BanquetOrderReview.banquet_order_id == oid
                )
            )
...
async def get_payment_method_monthly_trend(
    ...
    for p in payments:
        ...
        method_total[pm]    += 1
    monthly = [
        {"month": m, "methods": dict(month_method[m])}
        for m in sorted(month_method)
    ]
    return {
        "total_payments":    len(payments),
        "monthly":           monthly,
...
async def get_payment_installment_analysis(
    ...
    res2 = await db.execute(
        select(BanquetPaymentRecord).where(
            BanquetPaymentRecord.banquet_order_id.in_(order_ids)
        )
    )
    payments = res2.scalars().all()
    ...
    total = len(orders)
    installment_groups = [
        {"installments": k, "count": v, "pct": round(v / total * 100, 1)}
...
async def get_hall_revenue_efficiency(
    ...
    res2 = await db.execute(
        select(BanquetOrder).where(BanquetOrder.id.in_(all_order_ids))
    )
...
async def get_customer_lifetime_value(
    ...
    if top_n is None:
        top_n = 10
    if not orders:
        return {"total_customers": 0, "avg_clv_yuan": None, "avg_ltv_yuan": None, "top_customer": None, "top_customers": [], "top": []}
...
async def get_staff_order_load(
    ...
    staff = [
        {"user_id": uid, "order_count": len(oids), "task_count": staff_task_count[uid]}
...
async def get_order_upsell_rate(
    ...
    orders = res.scalars().all()
    if not orders:
        return {"total_orders": 0, "upsell_count": 0, "upsell_rate_pct": None,
                "avg_upsell_yuan": None}
    ...
    total = len(orders)
    avg_upsell = round(
        sum(o.total_amount_fen - upsell_threshold for o in upsell_orders) / upsell_count / 100, 2
    ) if upsell_count else None
    return {
        "total_orders":     total,
        "upsell_count":     upsell_count,
        "upsell_rate_pct":  round(upsell_count / total * 100, 1) if total else None,
...
async def get_payment_method_preference(
    ...
    res = await db.execute(
        select(BanquetPaymentRecord).join(
            BanquetOrder, BanquetPaymentRecord.banquet_order_id == BanquetOrder.id
        ).where(
            BanquetOrder.store_id == store_id,
            BanquetPaymentRecord.created_at >= cutoff,
        )
...
async def get_contract_signing_speed(
    ...
    rows = res.scalars().all()
    if not rows:
        return {"total_confirmed": 0, "total_won": 0, "with_contract": 0, "avg_signing_days": None, "avg_days_to_sign": None, "fast_sign_pct": None}
    ...
    order_ids = [o.id for o in orders]
    res2 = await db.execute(
        select(BanquetContract).where(BanquetContract.banquet_order_id.in_(order_ids))
    )
    contracts = res2.scalars().all()
    order_map = {o.id: o for o in orders}
    signing_days = []
...
async def get_order_lead_time_distribution(
    ...
    for o in orders:
        ...
        if diff <= 30:
            ...
            buckets["181天+"] += 1
    distribution = [
        {"bucket": b, "count": c, "pct": round(c / len(orders) * 100, 1)}
...
async def get_hall_revenue_per_day(
    ...
    res2 = await db.execute(
        select(BanquetOrder).where(BanquetOrder.id.in_(all_oids))
    )
    order_map = {o.id: o for o in res2.scalars().all()}
    halls = []
...
async def get_payment_collection_efficiency(
    ...
    orders = res.scalars().all()
    if not orders:
        return {"total_orders": 0, "total_receivable_yuan": None,
                "total_received_yuan": None, "collection_rate_pct": None}
...
async def get_banquet_type_table_trend(
    ...
    for btype, month_data in sorted(type_month_tables.items()):
        monthly = [
            {"month": m, "avg_tables": round(sum(ts) / len(ts), 1)}
            for m, ts in sorted(month_data.items())
        ]
        ...
        for hall in first_rows:
            ...
            for booking in hall_bookings:
                if "slot_date" not in getattr(booking, "__dict__", {}) or getattr(booking, "slot_date", None) is None:
                    order_res = await db.execute(
                        select(BanquetOrder).where(BanquetOrder.id == booking.banquet_order_id)
                    )
                    order = order_res.scalars().first()
                    booking.slot_date = getattr(order, "banquet_date", None)
                bookings.append(booking)
    if not bookings:
        return {"total_bookings": 0, "conflict_count": 0, "conflict_days": 0, "total_conflicts": 0, "conflict_rate_pct": None, "halls": []}
...
async def get_payment_overdue_analysis(
    ...
    orders = res.scalars().all()
    if not orders:
        return {"total_orders": 0, "overdue_count": 0, "overdue_rate_pct": None,
                "total_overdue_yuan": None}
    overdue = [
        o for o in orders
        if o.banquet_date < today and o.paid_fen < o.total_amount_fen
    ]
...
async def get_banquet_type_review_score(
    ...
    res = await db.execute(
        select(BanquetOrderReview).join(
            BanquetOrder, BanquetOrderReview.banquet_order_id == BanquetOrder.id
        ).where(
            BanquetOrder.store_id == store_id,
            BanquetOrderReview.created_at >= cutoff,
        )
...
async def get_exception_resolution_time(
    ...
    res = await db.execute(
        select(ExecutionException).join(
            BanquetOrder, ExecutionException.banquet_order_id == BanquetOrder.id
        ).where(
            BanquetOrder.store_id == store_id,
            ExecutionException.created_at >= cutoff,
        )
    ...
    resolved = [
        ...
        if hasattr(e, "resolved_at") and e.resolved_at is not None
    ]
    if not resolved:
        return {
            "total_exceptions": len(exceptions),
    ...
    days_list = [
        max(0.0, (e.resolved_at - e.created_at).total_seconds() / 86400)
        for e in resolved
    ]
    avg = round(sum(days_list) / len(days_list), 1)
    return {
...
async def get_order_cancellation_timing(
    ...
    n = len(orders)
    distribution = [
        {"bucket": b, "count": c, "pct": round(c / n * 100, 1)}
        for b, c in buckets.items() if c > 0
    ]
...
async def get_lead_budget_accuracy(
    ...
    avg_budget_yuan = round(sum((getattr(l, "expected_budget_fen", 0) or 0) for l in signed_leads) / len(signed_leads) / 100, 2) if signed_leads else None
    if not leads:
        return {"total_signed": 0, "avg_budget_yuan": None, "total_won": 0, "distribution": [], "avg_error_pct": None, "avg_deviation_pct": None, "accurate_pct": None}
    customer_ids = list({l.customer_id for l in won_leads if l.customer_id})
    if not customer_ids:
        return {
            "total_signed": len(signed_leads),
    ...
    distribution = [
        {"bucket": b, "count": c, "pct": round(c / len(errors) * 100, 1)}
        for b, c in buckets.items() if c > 0
    ]
...
async def get_staff_task_overdue_rate(
    ...
    tasks = res.scalars().all()
    if not tasks:
        return {"total_completed": 0, "total_tasks": 0, "overdue_count": 0, "overdue_tasks": 0, "overdue_rate_pct": None, "overall_overdue_rate_pct": None, "total_staff": 0, "staff": [], "by_staff": [], "highest_overdue_staff": None}
...
async def get_review_response_rate(
    ...
    res2 = await db.execute(
        select(BanquetOrderReview).where(
            BanquetOrderReview.banquet_order_id.in_(order_ids)
        )
    )
    ...
    monthly_list = [
        {
            ...
            "won":            monthly[ym]["won"],
            "conversion_pct": round(monthly[ym]["won"] / monthly[ym]["total"] * 100, 1)
            if monthly[ym]["total"] else 0,
        }
...
async def get_hall_booking_density(
    ...
    return {"total_halls": len(result_halls), "busiest_hall": result_halls[0]["hall_id"] if result_halls else None, "halls": result_halls, "overall_weekly_density": overall}
...
async def get_customer_avg_order_value(
    ...
    n = len(values)
    distribution = [
        {"tier": t, "count": c, "pct": round(c / n * 100, 1)}
...
async def get_customer_referral_rate(
    ...
    referral = sum(
        1 for l in leads
        if any(kw in (l.source_channel or "") for kw in referral_keywords)
    )
    total = len(leads)
    return {
        "total_leads":      total,
...
async def get_payment_partial_rate(
    ...
    total = len(orders)
    outstanding_fen = sum(
        max(0, o.total_amount_fen - o.paid_fen)
        for o in orders if o.paid_fen < o.total_amount_fen
    )
    return {
        "total_orders":      total,
...
async def get_staff_multitask_rate(
    ...
    multitask_count = sum(
        ...
        if any(c > 1 for c in staff_daily[uid].values())
    )
...
async def get_contract_amendment_speed(
    ...
    res2 = await db.execute(
        select(BanquetContract).where(
            BanquetContract.banquet_order_id.in_(order_ids)
        ).order_by(BanquetContract.created_at)
    )
    contracts = res2.scalars().all()
...
async def get_banquet_type_staff_ratio(
    ...
    res2 = await db.execute(
        select(ExecutionTask).where(
            ExecutionTask.banquet_order_id.in_(order_ids)
        )
    )
