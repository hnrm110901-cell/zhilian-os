...
async def create_dish(
    ...
    """创建菜品主档"""
    # 生成唯一编码
    count_result = await db.execute(select(func.count()).select_from(Dish).where(Dish.brand_id == brand_id))
...
async def list_dishes(
    ...
    total_result = await db.execute(select(func.count()).select_from(Dish).where(and_(*conditions)))
    total = total_result.scalar() or 0

    result = await db.execute(
        select(Dish).where(and_(*conditions)).order_by(Dish.created_at.desc()).offset((page - 1) * page_size).limit(page_size)
    )
...
async def get_dish(
    ...
    """获取菜品详情"""
    result = await db.execute(select(Dish).where(and_(Dish.id == dish_id, Dish.brand_id == brand_id)))
    ...
    cost_result = await db.execute(
        select(CostModel).where(CostModel.dish_id == dish_id).order_by(CostModel.calculated_at.desc()).limit(1)
    )
    cost = cost_result.scalars().first()

    # 获取试点状态
    pilot_result = await db.execute(
        select(PilotTest).where(PilotTest.dish_id == dish_id).order_by(PilotTest.created_at.desc()).limit(1)
    )
...
async def create_recipe_version(
    ...
    """创建配方版本（自动创建配方主档）"""
    # 确保配方主档存在
    recipe_result = await db.execute(select(Recipe).where(Recipe.dish_id == dish_id))
...
async def get_recipe_items(
    ...
    """获取 BOM 明细"""
    result = await db.execute(
        select(RecipeItem).where(RecipeItem.recipe_version_id == version_id).order_by(RecipeItem.sequence_no)
    )
...
async def create_pilot_test(
    ...
    """创建试点项目"""
    count_result = await db.execute(select(func.count()).select_from(PilotTest).where(PilotTest.brand_id == brand_id))
...
async def record_pilot_decision(
    ...
    result = await db.execute(select(PilotTest).where(and_(PilotTest.id == pilot_id, PilotTest.dish_id == dish_id)))
...
async def list_pilot_tests(
    ...
    result = await db.execute(
        select(PilotTest)
        .where(and_(PilotTest.dish_id == dish_id, PilotTest.brand_id == brand_id))
...
async def create_launch_project(
    ...
    """创建上市项目"""
    count_result = await db.execute(select(func.count()).select_from(LaunchProject).where(LaunchProject.brand_id == brand_id))
...
async def list_feedbacks(
    ...
    total_result = await db.execute(select(func.count()).select_from(DishFeedback).where(and_(*conditions)))
    total = total_result.scalar() or 0
...
async def list_retrospective_reports(
    ...
    """查询菜品复盘报告列表"""
    result = await db.execute(
        select(RetrospectiveReport)
        .where(and_(RetrospectiveReport.dish_id == dish_id, RetrospectiveReport.brand_id == brand_id))
...
async def get_dish_rd_dashboard(
    ...
    status_counts_result = await db.execute(
        select(Dish.status, func.count().label("cnt")).where(Dish.brand_id == brand_id).group_by(Dish.status)
    )
    ...
    pilot_result = await db.execute(
        select(func.count())
        .select_from(PilotTest)
        .where(and_(PilotTest.brand_id == brand_id, PilotTest.pilot_status == PilotStatusEnum.ACTIVE))
    )
    ...
    margin_result = await db.execute(
        select(func.avg(CostModel.margin_rate)).where(and_(CostModel.brand_id == brand_id, CostModel.calculated_at >= cutoff))
    )
