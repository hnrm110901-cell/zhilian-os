...
from src.models.agent_collab import AgentCollabSnapshot, AgentConflict, GlobalOptimizationLog
from src.models.user import User
from src.services.agent_collab_optimizer import AgentCollabOptimizer, AgentRecommendation, agent_collab_optimizer
...
async def list_conflicts(
    ...
    q = select(AgentConflict).where(AgentConflict.created_at >= since).order_by(desc(AgentConflict.created_at))
    if store_id:
...
async def collab_dashboard(
    ...
    conflict_rows_r = await db.execute(
        select(AgentConflict).where(AgentConflict.brand_id == brand_id, AgentConflict.created_at >= since)
    )
    ...
    opt_rows_r = await db.execute(
        select(GlobalOptimizationLog).where(
            GlobalOptimizationLog.brand_id == brand_id, GlobalOptimizationLog.created_at >= since
        )
    )
