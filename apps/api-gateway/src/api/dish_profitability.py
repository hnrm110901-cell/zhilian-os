...
async def list_dishes(
    ...
    dishes = await get_dish_profitability(db, store_id, period,
                                          bcg_quadrant=bcg_quadrant, limit=limit)
...
async def top_dishes(
    store_id: str,
    period: str = Query(..., description="YYYY-MM"),
    metric: str = Query("gross_profit_yuan",
                        description="gross_profit_yuan/revenue_yuan/order_count/"
    ...
    return {"store_id": store_id, "period": period, "metric": metric,
...
async def meta_quadrants():
    return {
        "quadrants": [
            {"key": q, "label": BCG_LABELS[q], "action": BCG_ACTIONS[q], "color": BCG_COLORS[q]}
