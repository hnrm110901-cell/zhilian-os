...
async def generate_breakdown(session_id: str, db: AsyncSession = Depends(get_db)):
    """Step 1: 生成拆细账快照"""
    svc = ReviewSessionService(db)
    try:
...
async def check_advance(session_id: str, db: AsyncSession = Depends(get_db)):
...
async def get_result_summary(session_id: str, db: AsyncSession = Depends(get_db)):
    """Step 5: 生成/获取闭环结果摘要"""
    svc = ReviewSessionService(db)
    try:
