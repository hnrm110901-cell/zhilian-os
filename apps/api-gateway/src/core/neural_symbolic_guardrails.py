...
class NeuralSymbolicGuardrails:
        ...
        return {
            ...
            "FIN_001": {
                ...
                "check": lambda proposal, context: (
                    proposal.get("total_amount", 0) <= context.get("monthly_budget", float("inf"))
                ),
            ...
            "FIN_002": {
                ...
                "check": lambda proposal, context: (
                    proposal.get("quantity", 0) <= context.get("historical_peak", float("inf")) * 1.2
                ),
            ...
            "FIN_003": {
                ...
                "severity": ViolationSeverity.CRITICAL,
                "check": lambda proposal, context: (
                    proposal.get("total_amount", 0) <= context.get("supplier_credit_limit", float("inf"))
                ),
            ...
            "OPS_001": {
                ...
                "severity": ViolationSeverity.CRITICAL,
                "check": lambda proposal, context: (proposal.get("staff_count", 0) >= context.get("minimum_staff", 0)),
            ...
            "OPS_002": {
                ...
                "category": RuleCategory.OPERATIONAL,
                "severity": ViolationSeverity.CRITICAL,
                "check": lambda proposal, context: (proposal.get("shift_hours", 0) <= 8),
            ...
            "OPS_003": {
                ...
                "severity": ViolationSeverity.CRITICAL,
                "check": lambda proposal, context: (
                    context.get("current_stock", 0) - proposal.get("transfer_quantity", 0) >= 0
                ),
            ...
            "SAFE_001": {
                ...
                "severity": ViolationSeverity.CRITICAL,
                "check": lambda proposal, context: (proposal.get("shelf_life_days", 0) >= 3),
            ...
            "SAFE_002": {
                ...
                "severity": ViolationSeverity.HIGH,
                "check": lambda proposal, context: (
                    not proposal.get("requires_cold_chain", False) or proposal.get("has_temperature_log", False)
                ),
            ...
            "COMP_001": {
                ...
                "severity": ViolationSeverity.CRITICAL,
                "check": lambda proposal, context: (proposal.get("supplier_certified", False)),
            ...
            "COMP_002": {
                ...
                "severity": ViolationSeverity.HIGH,
                "check": lambda proposal, context: (
                    abs(proposal.get("price", 0) - context.get("market_price", 0)) / context.get("market_price", 1)
                    <= float(os.getenv("GUARDRAIL_PRICE_DEVIATION_MAX", "0.3"))
                ),
            ...
            "BIZ_001": {
                ...
                "severity": ViolationSeverity.HIGH,
                "check": lambda proposal, context: (proposal.get("discounted_price", 0) >= context.get("cost_price", 0)),
            ...
            "BIZ_002": {
                ...
                "check": lambda proposal, context: (
                    not proposal.get("is_new_dish", False) or proposal.get("has_cost_breakdown", False)
                ),
            ...
            "REF_001": {
                ...
                "severity": ViolationSeverity.CRITICAL,
                "check": lambda proposal, context: (
                    proposal.get("refund_amount", 0) <= context.get("original_order_amount", float("inf"))
                ),
            ...
            "REF_002": {
                ...
                "severity": ViolationSeverity.HIGH,
                "check": lambda proposal, context: (
                    proposal.get("days_since_order", 0) <= int(os.getenv("GUARDRAIL_REFUND_WINDOW_DAYS", "7"))
                ),
            ...
            "REF_003": {
                ...
                "check": lambda proposal, context: (
                    ...
                    <= context.get("daily_revenue", 0) * float(os.getenv("GUARDRAIL_REFUND_DAILY_RATIO_MAX", "0.20"))
                ),
            ...
            "REF_004": {
                ...
                "severity": ViolationSeverity.HIGH,
                "check": lambda proposal, context: (
                    proposal.get("refund_amount", 0) <= int(os.getenv("GUARDRAIL_REFUND_SINGLE_LIMIT", "50000"))  # 单位：分
                ),
            ...
            "REF_005": {
                ...
                "severity": ViolationSeverity.MEDIUM,
                "check": lambda proposal, context: (
                    context.get("customer_refund_count_24h", 0) < int(os.getenv("GUARDRAIL_REFUND_CUSTOMER_FREQ_LIMIT", "3"))
                ),
                "threshold_key": "customer_refund_count_24h",
            },
        }

    def validate_proposal(self, ai_proposal: AIProposal, context: Dict[str, Any]) -> GuardrailResult:
        """
        校验AI提案
            ...
            try:
                ...
                if not passed:
                    ...
                    violation = RuleViolation(
                        ...
                        actual_value=ai_proposal.content.get(rule.get("actual_key", "value")),
                        threshold_value=context.get(rule["threshold_key"]),
                        recommendation=self._generate_recommendation(rule_id, ai_proposal, context),
                    )
        ...
        if requires_approval:
            ...
            if critical_violations:
                escalation_reason = f"检测到{len(critical_violations)}个严重违规，" f"必须人工审批"
            else:
    ...
    def _requires_human_approval(self, violations: List[RuleViolation]) -> bool:
        """判断是否需要人类审批"""
        # 任何CRITICAL违规都需要审批
    ...
    def _generate_recommendation(self, rule_id: str, ai_proposal: AIProposal, context: Dict[str, Any]) -> str:
        """生成改进建议"""
        recommendations = {
        ...
        return recommendations.get(rule_id, "建议人工审核并调整方案")

    def auto_fix_proposal(
        self, ai_proposal: AIProposal, violations: List[RuleViolation], context: Dict[str, Any]
        ...
        fixable_violations = [v for v in violations if v.severity in [ViolationSeverity.MEDIUM, ViolationSeverity.LOW]]
        ...
        for violation in fixable_violations:
            if violation.rule_id == "FIN_002":
                # 自动调整采购量至历史峰值的120%
                modified["quantity"] = int(context.get("historical_peak", 0) * 1.2)
