...
async def test_optimize_schedule_success(schedule_agent, mock_rag_service):
    """Test schedule optimization with Tool Use"""
    from src.agents.llm_agent import AgentResult
    schedule_agent.rag_service = mock_rag_service

    mock_result = AgentResult(success=True, data="建议排班12人，午高峰(11:00-13:00)需6人，晚高峰(17:00-19:00)需5人。", confidence=0.85)
    with patch.object(schedule_agent, 'execute_with_tools', new=AsyncMock(return_value=mock_result)):
...
async def test_optimize_schedule_no_expected_flow(schedule_agent, mock_rag_service):
    ...
    mock_result = AgentResult(success=True, data="排班建议", confidence=0.8)
    with patch.object(schedule_agent, 'execute_with_tools', new=AsyncMock(return_value=mock_result)):
        result = await schedule_agent.optimize_schedule(
            store_id="STORE001",
            date="2026-02-22",
...
async def test_predict_staffing_needs_success(schedule_agent, mock_rag_service):
    """Test staffing needs prediction with Tool Use"""
    from src.agents.llm_agent import AgentResult
    schedule_agent.rag_service = mock_rag_service

    mock_result = AgentResult(success=True, data="未来7天建议人数: 周一10人, 周二11人, 周三12人...", confidence=0.85)
    with patch.object(schedule_agent, 'execute_with_tools', new=AsyncMock(return_value=mock_result)):
...
async def test_analyze_shift_efficiency_success(schedule_agent, mock_rag_service):
    """Test shift efficiency analysis with Tool Use"""
    from src.agents.llm_agent import AgentResult
    schedule_agent.rag_service = mock_rag_service

    mock_result = AgentResult(success=True, data="班次效率良好，人均服务25位客户，建议优化点餐流程。", confidence=0.85)
    with patch.object(schedule_agent, 'execute_with_tools', new=AsyncMock(return_value=mock_result)):
...
async def test_balance_workload_success(schedule_agent, mock_rag_service):
    """Test workload balancing with Tool Use"""
    from src.agents.llm_agent import AgentResult
    schedule_agent.rag_service = mock_rag_service

    staff_ids = ["STAFF001", "STAFF002", "STAFF003"]
    mock_result = AgentResult(success=True, data="员工A工作量偏高，建议调整班次分配，平衡工作时长。", confidence=0.85)
    with patch.object(schedule_agent, 'execute_with_tools', new=AsyncMock(return_value=mock_result)):
