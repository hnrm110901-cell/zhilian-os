...
const DashboardTab: React.FC<{ storeId: string }> = ({ storeId }) => {
  ...
  const fetch = useCallback(async () => {
    setLoading(true);
    try {
      const d = await apiClient.get(`/api/v1/fct/${storeId}/dashboard`);
      setData(d);
    } catch {
...
const TaxTab: React.FC<{ storeId: string }> = ({ storeId }) => {
  ...
  const estimate = async () => {
    ...
    try {
      const d = await apiClient.get(
        `/api/v1/fct/${storeId}/tax/${year}/${month}`,
  ...
  const save = async () => {
    ...
    try {
      await apiClient.post(
        `/api/v1/fct/${storeId}/tax/${year}/${month}/save`,
        null,
        { params: { taxpayer_type: type } },
      );
...
const CashFlowTab: React.FC<{ storeId: string }> = ({ storeId }) => {
  ...
  const forecast = async () => {
    setLoading(true);
    try {
      const d = await apiClient.get(`/api/v1/fct/${storeId}/cash-flow`, {
...
const BudgetTab: React.FC<{ storeId: string }> = ({ storeId }) => {
  const [year,  setYear]  = useState(DEFAULT_YEAR);
  const [month, setMonth] = useState(DEFAULT_MONTH);
  const [data,  setData]  = useState<any>(null);
  const [loading, setLoading] = useState(false);
  ...
  const load = async () => {
    setLoading(true);
    try {
      const d = await apiClient.get(`/api/v1/fct/${storeId}/budget-execution/${year}/${month}`);
      setData(d);
    } catch {
