...
function MoverList({ storeId, period }: { storeId: string; period: string }) {
  ...
  const gainOption = gainers.length ? {
    ...
    series: [
      ...
      { name: '销量效应', type: 'bar', stack: 'total', data: gainers.map(r => r.volume_effect_yuan).reverse(), itemStyle: { color: '#FF6B2C' } },
  ...
  const lossOption = losers.length ? {
    ...
    series: [
      ...
      { name: '销量效应', type: 'bar', stack: 'total', data: losers.map(r => r.volume_effect_yuan), itemStyle: { color: '#FF6B2C' } },
